...
3. cat 错标 (industry-legal.json 中 cat 字段)
...
FP = re.compile(r"统一复核|降低上手门槛|可追溯流程|边界样本建议单独标注|减少重复确认成本|"
                ...
                r"复用模板示例|保留复用模板")
# desc-en 占位
DESC_EN_PH = re.compile(r'free online tool|free tool|generate results online', re.I)
...
# 全站性：body 里 legal 键但 legal 目录无
bkeys = set(body.keys())
for slug in tools:
    bkeys.discard(slug)
# 同时剔除全站其它目录同名（已归 all_basenames）
