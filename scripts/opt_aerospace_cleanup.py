#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""清理 aerospace 源 html 中 opt-guide/opt-faq 可见旧套话区块（三处同清之一）。
- 仅 lift-coefficient / rocket-delta-v 这 2 个文件含硬编码可见套话（其余 35 个文件的深度内容由 _build.py 从 content_deepdive 重建，build 后自动变真实，无需手动清）。
...
CATS = ["aerospace"]
CLICHE = ("工作与生活中的相关计算", "在对应的输入框或选项中填写、选择所需参数。")
