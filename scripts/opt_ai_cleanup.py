#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""清理 ai 源 html 中可见旧套话（三处同清之一：可见 opt 区块 + formula-desc）。
- 第一阶段：含"在对应的输入框或选项中填写、选择所需参数。"的文件（6 个，均带旧 FAQPage LD）
   → opt-faq section 整段替换为 content_deepdive 真实 faqs；
   → opt-guide"适用场景"套话段落替换为 scenarios[0]；
   → 使用说明短语规范化为"按页面提示逐项填写或选择所需参数。"。
- 第二阶段：formula-desc 含模板套话（"工具名称："/"本计算器基于标准数学运算"等）的 3 个文件
   → 替换为该工具真实计算原理说明。
- 其余 58 个文件仅 deep-dive 套话，由 _build.py 从 content_deepdive 重建时自动变真实，无需手动清。
"""
...
FORMULA_DESC = {
    "ai-code-review": "本工具在浏览器本地用规则与启发式对 JavaScript/TypeScript 做静态分析，识别未声明变量、冗余分支、可疑空值解引用等模式并给出提示；纯前端运行，代码不上传服务器。",
    "ai-text-summarizer": "本工具基于 TextRank 算法在浏览器本地对句子构图（句间相似度为边、按重要度排序）抽取关键句生成摘要；纯前端运行，文本不上传服务器。",
    "ai-prompt-generator": "本工具按所选场景与角色、任务、约束等字段套用结构化模板拼接生成 Prompt；纯前端拼接，内容不上传服务器。",
}
FD_CLICHE = ("工具名称：", "本计算器基于标准数学运算", "本工程计算", "本工具用于", "本计算器基于")
