...
const CASES = [
  ...
  // ── BATCH156：列表统计类（CSAT / 响应时长）锁「多派生量同一次注入」──────
  {
    ...
    expect: ["平均分： 3.40 / 5", "各档分布：1分 2 · 2分 0 · 3分 0 · 4分 0 · 5分 3", "CSAT 满意度（≥3分）： 60.0%"],
    ref: "注入 1,1,5,5,5 + 阈值 3 ⇒ n=5、sum=17、均值 3.40、众数 5、分布 1 分 2 个 / 5 分 3 个、达标 3 份 60.0%，三个锚同一次注入产出（默认态是 10 份 / 4.00 / ≥4 分 70.0% / 分布 0·1·2·3·4），任一拆解错都会变红。",
  },
  {
    ...
    expect: ["平均响应时长： 28.75 秒", "中位响应时长： 15.00 秒", "达标率 50.0%"],
    ref: "注入 5,10,20,80 + 阈值 15 秒（默认 20,45,75,30,120 / 阈值 60）⇒ total=115、均值 28.75、偶数个取中间两数均值 (10+20)/2=15.00、达标 2 条 50.0%。fmt 固定两位小数，故中位串是 15.00 而非 15。",
  },
];
