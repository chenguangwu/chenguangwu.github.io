...
{
  // 处方差值/描述统计（页面名为「比例分析」但实现为通用描述统计，见 DEV-PLAN §10.6 缺陷 D 同类）
  // 数据集 5,8,12,17,21,30 → n=6、总和 93.00、均值 15.50、中位 14.50、极差 25.00、方差 70.25、标准差 8.38
  "slug": "tcm-pharmacy/analysis-ratio-prescription",
  "inputs": {
    "data": "5,8,12,17,21,30"
  },
  "expect": [
    "方差： 70.25",
    "标准差： 8.38"
  ],
  "ref": "独立复算（总体方差，除 n）：n=6、sum=93→93.00、mean=15.50、median=(12+17)/2=14.50、min=5、max=30、range=25.00、variance=Σ(x-15.5)²/6=421.5/6=70.25、std=√70.25=8.3815→8.38。默认数据集 10,20,…,80 → 均值 45.00、方差 525.00、标准差 22.91，与本例两锚点不同。原 inputs「10,20,…,80_X」与 expect「80_X」是 textarea 自身 value 的**回显**（collectStrings 会采集元素 value），calc 即便全坏也 PASS，属无效用例"
},
{
