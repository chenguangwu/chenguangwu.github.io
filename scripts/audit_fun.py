...
# ---- 5. formula 缺口 ----
print("\n【5】formula 缺口 (按 cat 区分：计算类必须有框；game/generate 豁免)")
# 计算类 = 有明确计算/换算口径的工具；游戏与生成器无计算公式，按 §4.1.2 属合理豁免
CALC_CATS = {'calculator', 'convert', 'health'}
fb_missing, fb_exempt = [], []
for slug in tools:
    s = open(os.path.join(TOOLS, slug + '.html'), encoding='utf-8').read()
    if 'class="formula-box"' in s:
        continue
    if ind_by_file.get(slug, {}).get('cat') in CALC_CATS:
        fb_missing.append(slug)
    else:
        fb_exempt.append(slug)
calc_total = sum(1 for slug in tools if ind_by_file.get(slug, {}).get('cat') in CALC_CATS)
print(f"  计算类工具={calc_total}  计算类缺框={len(fb_missing)}  豁免(游戏/生成器)={len(fb_exempt)}")
