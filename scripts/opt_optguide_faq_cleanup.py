#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""清理已优化 4 分类（accounting/acoustics/acupuncture/admin）源 html 中
opt-guide/opt-faq 可见区块残留的旧套话（早期 opt_content.py 批量生成的可见内容，
与旧 FAQPage LD 同源，之前只清了 JSON-LD 漏了可见区块）。
...
- 不改 calc 关键词、不碰 deep-dive（由 _build.py 按 content_deepdive 重建）、不动 opt-guide 的"如何使用/参数说明"。
"""
import re, glob, json, os, sys

DATA = "i18n/tools/content_deepdive.json"
CATS = ["accounting", "acoustics", "acupuncture", "admin"]
