# -*- coding: utf-8 -*-
"""acoustics 28 工具：删旧套话 FAQPage LD + 注入真实 FAQPage LD + title 加"免费"。

纪律：
- 旧套话 FAQPage LD 必须先行删除，否则注入幂等判断 if 'FAQPage' in s 会跳过。
- 真实 FAQPage LD 从 content_deepdive.json 的 faqs 生成（与 deep-dive 真实 FAQ 一致）。
- 注入位置必须用负向后顾 (?<!['"\w])</body> 只匹配【文档级】</body>，
  排除 JS 字符串字面量内的同名标签，避免把 JSON 塞进函数 <script> 块。
- 源 html title 的"免费"会被 _build.py 规范化覆盖，已在 _build.py 对 acoustics 分类追加；
  此处仍注入源 title 以保持语义一致，最终以 build 产物为准。
"""
import json, os, re, sys

ROOT = "tools/acoustics"
FREE = "（免费）"
...
def process(path, slug, dry):
    ...
    # 3) SEO: title / og:title / twitter:title 加"免费"
    def add_free(m):
        t = m.group(1)
        if FREE in t:
            return m.group(0)
        return "<title>%s%s - ToolBox</title>" % (t, FREE)
    s2, n1 = re.subn(r"<title>(.*?) - ToolBox</title>", add_free, s, count=1)
    def add_free_attr(m):
        pre, val, post = m.group(1), m.group(2), m.group(3)
        if FREE in val:
            return m.group(0)
        return "%s%s%s%s" % (pre, val, FREE, post)
    s2, n2 = re.subn(r'(<meta property="og:title" content=")([^"]*)(")', add_free_attr, s2, count=1)
    s2, n3 = re.subn(r'(<meta name="twitter:title" content=")([^"]*)(")', add_free_attr, s2, count=1)
    if n1 or n2 or n3:
        report.append("seo-title(%d/%d/%d)" % (n1, n2, n3))
    s = s2
...
def main():
    ...
    files.sort()
    print("processing %d acoustics tools (dry=%s)" % (len(files), dry))
    for f in files:
        slug = f[:-5]
