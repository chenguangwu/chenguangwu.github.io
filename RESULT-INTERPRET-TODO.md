...
- [x] #1 `tools/clinical-nursing/pain-nrs.html` — 疼痛数字评分 (NRS) 与表情对照器（输入0） 【排除:页内已含 NRS 0-10 分级参考(info-box),避免重复注入】
- [x] #2 `tools/edu/capital-quiz.html` — 首都测验（输入0） 【排除:测验类,结果为答题数,无领域解读意义(避免伪功能)】
- [x] #3 `tools/edu/math-quiz.html` — 数学测验（输入0） 【排除:测验类,同上】
- [x] #4 `tools/fun/coin-flip.html` — 抛硬币（输入0） 【排除:概率游戏,结果为正反面,无解读意义】
- [x] #5 `tools/fun/color-guess.html` — 猜颜色（输入0） 【排除:记忆游戏,无解读意义】
- [x] #6 `tools/fun/color-memory.html` — 颜色记忆（Simon）（输入0） 【排除:记忆游戏(Simon),无解读意义】
- [x] #7 `tools/fun/pattern-memory.html` — 图案记忆（输入0） 【排除:图案记忆游戏,无解读意义】
...
- [x] #11 `tools/tcm-diagnosis/disease-nature.html` — 病性辨析器（六淫）（输入0） 【排除:中医六淫辨证,非现代医学循证分级,按需另排】
...
- [x] #18 `tools/chinese-cook/cutting-sizes.html` — 切配尺寸对照（输入1）

## 批次 2（18 个）

- [x] #19 `tools/chinese-cook/ingredient-substitute.html` — 食材替代查询（输入1）
- [x] #20 `tools/chinese-cook/oil-temp.html` — 油温烹饪指南（输入1）
- [x] #21 `tools/data/chart-generator.html` — 图表生成器（输入1）
- [x] #22 `tools/finance/credit-card-bin.html` — 信用卡 BIN 查询（输入1）
...
- [x] #27 `tools/finance/tax-bracket.html` — 税率档次速查（输入1）
- [x] #28 `tools/fun/keyboard-heatmap.html` — 键盘热力图（输入1） 【排除:可视化展示,无数值结果解读】
- [x] #29 `tools/fun/roulette-simulator.html` — 轮盘模拟器（输入1） 【排除:概率游戏模拟,无解读意义】
- [x] #30 `tools/gardening/pot-capacity.html` — 花盆容量计算（输入1）
- [x] #31 `tools/it/html-nesting-checker.html` — HTML 嵌套检查器（输入1）
- [x] #32 `tools/it/pdf-signature-checker.html` — PDF 签名检查（输入1）
- [x] #33 `tools/it/text-statistics.html` — 文本统计分析（输入1）
...
- [x] #37 `tools/math/factorial-calc.html` — 阶乘计算器（输入1）
...
- [x] #42 `tools/quantum/angular-momentum-quant.html` — 由主量子数求角动量（输入1）
...
- [x] #49 `tools/science/physics-calculator.html` — 物理计算器（输入1） 【排除:动态生成公式/输入,无固定结果容器,通用解读不适用】
...
- [x] #54 `tools/tcm-diagnosis/meridian-differentiation.html` — 经络辨证疼痛对应器（输入1） 【排除:中医经络辨证,非现代循证】

## 批次 4（18 个）

- [x] #55 `tools/urology/iief5-score.html` — 勃起功能(IIEF-5)评分器（输入1）
- [x] #56 `tools/yi/yi-divination.html` — 周易占卜（输入1） 【排除:周易占卜,非科学评估,不写伪解读】
- [x] #57 `tools/accounting/debt-service-coverage.html` — 由经营现金流与债务偿付额求保障倍数（输入2）
- [x] #58 `tools/ballistics/caliber-conversion.html` — 口径换算器（输入2）
- [x] #59 `tools/banking/fisher-real-rate.html` — 费雪实际利率计算器（输入2）
- [x] #60 `tools/biz/superscript-text.html` — 上标文字（输入2） 【排除:文本格式转换,无计算数值解读】
- [x] #61 `tools/blasting/delay-blasting.html` — 微差延迟优化器（输入2） 【排除:需专业爆破工艺参数,通用解读易误导】
- [x] #62 `tools/cardiology/statin-dose.html` — 他汀(降脂幅度)剂量换算器（输入2）
...
- [x] #71 `tools/energy/air-purifier-area.html` — 空气净化器适用面积计算（输入2）
- [x] #72 `tools/energy/electrical-power.html` — 由电压与电流求电功率（输入2）

## 批次 5（18 个）

- [x] #73 `tools/energy/joule-heating.html` — 由电流与电阻求热功率（输入2）
- [x] #74 `tools/futures/option-payoff.html` — 期权盈亏图（输入2）
- [x] #75 `tools/geometry/pyramid-volume.html` — 由底面积与高求棱锥体积（输入2）
- [x] #76 `tools/it/bip39-generator.html` — BIP39 助记词生成器（输入2）
- [x] #77 `tools/it/calc-1.html` — 文件大小单位换算（输入2）
- [x] #78 `tools/it/calc-4.html` — CSS单位换算（px / em / rem / pt / %）（输入2）
...
- [x] #89 `tools/life/radiation-converter.html` — 辐射剂量换算器（输入2）
...
- [x] #95 `tools/medical2/medical-abbrev.html` — 医学术语缩写（输入2） 【排除:缩写查询工具,结果为释义非计算得出】
- [x] #96 `tools/meteorology/wet-bulb-temperature.html` — 湿球温度 T_w（输入2）
...
- [x] #99 `tools/ophthalmology/visual-acuity-converter.html` — 视力表(Snellen/logMAR)换算器（输入2）
- [x] #100 `tools/optical/blue-light-filter.html` — 防蓝光透射比计算器（输入2）
...
- [x] #111 `tools/robotics/end-effector-reach.html` — 二连杆工作空间计算器（输入2）
...
- [x] #119 `tools/textile2/dye-temp.html` — 染色温度对照表（输入2）
