...
- [ ] **`it/jwt-parser` intro `<p>` 不在 `.tool-card-accent` 内 —— 2026-09-16 新发现，未修**：该页 intro p 位于第 195 行起的普通 `.card` 内，而 `js/tool-i18n.js:400` 只在 `.tool-card-accent`（行 136）里 `querySelector('p')`，因此其中文 data-zh **永不被回写**，中文用户看到英文 intro（无头 Chrome 实测）。修法须调整页面 DOM 结构（或放宽 i18n 选取器），影响面待评估，**未擅自改动**。
