...
| # | 分类 | 工具数 | # | 分类 | 工具数 |
|---|---|---|---|---|---|
| 1 | life | 73 | 9 | marketing | 46 |
| 2 | biz | 69 | 10 | surveying | 44 |
| 3 | fun | 65 | 11 | meteorology | 42 |
| 4 | ai | 64 | 12 | (按同口径递减) | — |
| 5 | agriculture | 63 | | | |
| 6 | hydraulic | 56 | | | |
| 7 | automotive | 55 | | | |
...
> 全站 209 个分类 / 4767 个工具。上表为**剩余待收口分类**（按工具数降序）；已收口的 `it`（336）、`general`（183）、`design`（109）、`finance`（104）、`science`（98）、`sports`（78）六项已按「完成一个删一个」移除，其余分类按同口径递减。

> **收口状态（2026-09-19）**：`it` ✅ 336/336 A；`general` ✅ 183/183 A；`design` ✅ 109/109 A；`finance` ✅ 104/104 A；`science` ✅ 98/98 A；`sports` ✅ 八维审计全绿（cat 9 错标修 / 英文占位 0 / formula 55/55；28 非 A 为历史存量，属 P2 C→A 待提升）。**下一个待收口分类 = `life`（73 工具）。**
