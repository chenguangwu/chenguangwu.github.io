...
### ✅ gardening2（5 键，真实化收口，最新一条）
- 5 个工具全部真实化（fertilizer-ppm 求用量 目标ppm×水量÷养分小数÷1e6×1000 与反算 ppm=用量×养分小数×1e6÷水量÷1000，附稀释倍数=水量L×1000÷用量g、滴数=用量ml÷0.05 / pruning-time 18 种花木三档筛选(花后10·休眠6·花前2)参考型 / watering-frequency 间隔天数=季节基准×天气×土壤×植物×位置五系数连乘夹1~30天 + 单次量=盆径²×0.8ml + 月用水量 / pest-control 12 条病虫害(药剂·间隔·最多次数/季·安全采收期·安全等级)参考型 / lawn-height 8 种草坪草四季推荐高度+间隔+最大高度与 1/3 原则 参考型），每个含 3 真实场景 + 1 算例或操作示例 + 2 专业 FAQ。
- 算例数字全部 node 实跑复核（/tmp/verify_gardening2.js）：ppm 200ppm/1L/20%→1 g·1000倍·20滴、150/10/20%→7.5 g·1333倍·150滴、100/20L/15%→13.333 g·1500倍·267滴，反算 1g/1L/20%→200、7.5g/10L/20%→150；watering 春/晴/沙质/观叶/室内/15cm→2.184→2天·180ml·月2.7L·需频繁浇水，夏/高温/沙质/仙人掌/室外/15→2.24→2天、夏/高温/沙质/多肉/室外/20→1.4→1天·320ml·月9.6L、冬/雨/黏土/仙人掌/室内→101.92→封顶30天·月0.2L·少量浇水、秋/阴/壤土/观花/阳台/30→4.32→4天·720ml·月5.4L·正常浇水；lawn 夏季占比 狗牙根3÷5=60.0%·结缕草3.5÷5.5=63.6%·早熟禾5÷7=71.4%·高羊茅6÷8=75.0%·剪股颖2÷3=66.7%·海滨雀稗2.5÷4=62.5%，春季 狗牙根2.5÷5=50.0%·高羊茅5÷8=62.5%。
- pest-control 含旧版 opt-guide/opt-faq 套话块(2块)+FAQPage LD 旧套话，经 opt_faq_ld_sync(--cat gardening2 同步2条)+opt_cleanup_opt_blocks(--cat gardening2 删2块)清零（LD JSON 校验合法）；其余 4 文件原 LD 干净。五道门禁全过（含清理后复跑）；六型指纹(含"先统一"变体)+可见 opt 块扫描 0 残留；真实关键词 ppm1/pruning10/watering1/pest16/lawn4；键数守恒 5022。脚本：scripts/apply_gardening2.py。

## 八、当前进行中分类

> 当前无进行中分类。新分类开工时先在此登记（目录 + 工具数），收口后写入第七节并覆盖旧归档记录（§4.3 第 4 步）。

## 九、分类总清单（待办，完成一个删一个；剩 35 个目录）
