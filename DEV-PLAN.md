...
### ✅ logistics2（5 键，真实化收口，最新一条）
- 5 个工具全部真实化（inventory-aging 日均出库=出库量÷周期→周转天数=库存量÷日均出库→库存金额→呆滞占比(阈值判定、0出库记∞) / picking-route S型(通道数×长) vs U型(2×通道数×长)+共用横向(通道数−1)×间距，行走÷速度+拣货点×单点 / loading-utilization 3 取向 floor 取最大为体积上限、重量上限=floor(载重÷单重)、maxQty=min 两者、超装/超载/接近满载判定 / storage-allocation ABC 累计占比 70%/90% 分档 + 按 SKU 数占比分摊排数 + 门口位置镜像分区 / packaging-cushion t=C×h÷G×重量修正(1+ln(w+1)×0.08) 与五材料系数+脆值分级+超能力提示），每个含 3 真实场景 + 1 算例 + 2 专业 FAQ。
- 算例数字全部 node 实跑复核（/tmp/verify_logistics2.js）：aging 矿泉水5000/12000/2.5、纸巾2000/3000/5、旧款锅具800/50/80（周期90·阈值180）→日均133.33/33.33/0.56、周转37.5/60.0/1440.0天、金额12500/10000/64000、呆滞1个·64000元·占比74.0%；picking 8通道/20m/3m/1.2/10s/3点→拣货点24·拣货240s、S 181.0m·150.83s·总390.83s、U 341.0m·284.17s·总524.17s、推荐S省133.33s(25.4%)，4通道/30m/15s/5点→407.5 vs 507.5 省100.0s(19.7%)；loading 6.8×2.3×2.4(37.536m³)/5000kg、箱0.6×0.4×0.5(0.12m³)/15kg、计划500件→取向(0.4,0.5,0.6)得17×4×4=272、重量上限333、maxQty272体积受限、利用率87.0%/81.6%，计划500件需60.00m³(159.8%)+7500kg(150%)超装超载，改300件→36.00m³(95.9%)/4500kg(90.0%)仍超上限；storage 8 SKU(1200/900/700/500/300/200/120/80，总4000)→A3(2800·70.0%)/B2(800·20.0%)/C3(400·10.0%)、10排门前端→A第1~4·B第5~6·C第7~10（门后端镜像）；cushion 3kg/80cm/60g→修正系数1.1109、EPS4.74cm(总9.5)/EPE3.85/PU3.26/气泡膜5.63(15层)/纸浆5.92(超5.0能力上限)、静应力0.0100kg/cm²、脆值60判易碎；10kg/120cm/40g EPS→11.44cm 超8.0 需换PU或组合。
- logistics2 5 个源 HTML 原 LD 即干净、无 opt-guide 旧套话（占位/opt块/LD 三处扫描全 0），无需清理。五道门禁全过；六型指纹(含"先统一"变体)+可见 opt 块扫描 0 残留；真实关键词 aging1/picking1/loading1/storage3/cushion5；键数守恒 5022。脚本：scripts/apply_logistics2.py。

## 八、当前进行中分类

> 当前无进行中分类。新分类开工时先在此登记（目录 + 工具数），收口后写入第七节并覆盖旧归档记录（§4.3 第 4 步）。

## 九、分类总清单（待办，完成一个删一个；剩 36 个目录）
