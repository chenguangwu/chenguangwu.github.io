...
| # | 分类 | 工具数 | # | 分类 | 工具数 |
|---|---|---|---|---|---|
| 1 | fun | 65 | 8 | statistics | 51 |
| 2 | ai | 64 | 9 | edu | 49 |
| 3 | agriculture | 63 | 10 | marketing | 46 |
| 4 | hydraulic | 56 | 11 | surveying | 44 |
| 5 | automotive | 55 | 12 | meteorology | 42 |
| 6 | legal | 54 | 13 | (按同口径递减) | — |
| 7 | realestate | 54 | | | |

> 全站 209 个分类 / 4767 个工具。上表为**剩余待收口分类**（按工具数降序）；已收口的 `it`（336）、`general`（183）、`design`（109）、`finance`（104）、`science`（98）、`sports`（78）、`life`（73）、`biz`（69）八项已按「完成一个删一个」移除，其余分类按同口径递减。

> **收口状态（2026-09-19）**：`it` ✅ 336/336 A；`general` ✅ 183/183 A；`design` ✅ 109/109 A；`finance` ✅ 104/104 A；`science` ✅ 98/98 A；`sports` ✅ 八维审计全绿；`life` ✅ 八维审计全绿；`biz` ✅ 八维审计全绿（cat 3 错标修 / 69/69）。**下一个待收口分类 = `fun`（65 工具）。**
