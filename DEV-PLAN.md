...
- **harness 输入桩覆盖现状（2026-09-24 更新）**：
  ...
  - ✅ **坏用例第三形态：`inputs` 键是生成器模板串残留 ⇒ 两套扫描器同时失灵（BATCH101 闭环，2026-09-24）**：`home` 5 例（`furniture-layout` / `lighting-calculator` / `paint-calculator` / `renovation-budget` / `room-calculator`）、`hotel` 3 例（`currency-exchange` / `itinerary-planner` / `occupancy-revpar`）、`science` 4 例（`newtons-second` / `kinetic-energy` / `pendulum-period` / `molar-mass-calculator`）—— `all_default` 67→**56**、`no_inputs` 98 不变；判别器已检 2961→**2972** / 跳过 200→**189**。**本批四项新认知**：① **「正确变红」≠「有判别力」** —— `home/room-calculator` 的旧 `inputs` 键是生成器模板串残留（`${f}` 与 `${f==='radius'?3:5}_X`）、`hotel/itinerary-planner` 的 `pri${i}`，页面无此 id、expect 也无从产生；而 `weakKind` 见「inputs 非空但 `_pageDefaults` 取不到该键 ⇒ return null」⇒ **既不进弱用例棘轮**，`discriminate_check` 又因 expect 永不命中而记为「正确变红」⇒ **两把锁同时失灵、静默失效**。② **运行期生成控件必须走 `clicks`**：`room-calculator` 的 `shape-input`、`itinerary-planner` 的 `spotList`（`spot{i}`/`pri{i}`）控件由 JS 拼 `innerHTML` 生成、不在静态 HTML 内 ⇒ 注入 `inputs` 既触发不了内联 `oninput`、判别器也取不到默认值。③ **canvas 型页面无文本输出**：`furniture-layout` 的房间尺寸只写进 `canvas.style.width/height`（边长×SCALE 60）、家具名只经 `createElement+appendChild` 反映进容器 `innerHTML`；原 expect「橱柜」来自 `renderFurnList()` 用常量 `FURNITURE` 渲染的静态按钮列表（与输入完全无关）。④ **localStorage 唯一数据源 + 空数组 fallback 的页面有出路**：`renovation-budget` 可在 `clicks` 内覆写 `localStorage.getItem` 注入数据（等价于「用户本就有数据」），`medical2` / `travel` / `cleaning` 当年判「结构性不可注入」时未尝试此法。**同批修掉 1 个页面真缺陷**：`tools/science/molar-mass-calculator.html` 的元素组成「占比」列分母用了循环内**累加中的前缀和**⇒ 首行恒 100.00%（H2O 合计 188.81%、C6H12O6 合计 167.66%，占比合计 >100% 物理不可能），已改为「先累加求 total、再第二趟算占比」。**工具坑（复审）**：`patch4.py`（无引号键风格渲染器）会吃掉**被改块的块前注释**、把下一块的 `{` 去缩进、并 normalize 块间空行 ⇒ **能用 Edit 就别用正则替换器**。
  - ✅ **锚点四则：静态表档位词 / 结论句 / 同值陷阱 / 零影响键（BATCH102 闭环，2026-09-25）**：`misc2` 4 例 + `logistics2` 4 例 + `media` 3 例去默认化（`all_default` 56→**45**、`no_inputs` 98 不变；判别器已检 2972→**2983** / 跳过 189→**178**）。① **静态参考表/说明列的档位词一律不能用** —— `cigarette-tar` 原 expect「中焦油」出自 `renderTable()` 的静态分级表（≤5 极低 / 5–8 低 / 8–12 中 / >12 高，默认态同样渲染）、`tax-refund`「日元可退税」是国家表说明列；② **data-card 标签与结论句同样是常量**（`car-residual`「估算残值」、`loading-utilization`「建议装载」、`packaging-cushion`「跌落下不受损」、`assessor-25`「复制到后续内容」、`tester-18`「继续迭代测试」）；③ **同值陷阱**：`inventory-aging` 的「呆滞品数 4 / 呆滞金额占比 77.0%」在 period=90 与 200 两态**完全相同**（呆滞判定只看 threshold），只能锚随 period 变的周转天数派生列（`60.0 83 ¥12,500`）；④ **零影响键不要写进 inputs** —— `car-residual` 的 `life`/`salvage` 只在 straight/double 方法下参与计算，rule（默认）模式下改了输出不变，写进 inputs 会造成「看似非默认」的假象。**阈值翻转法**：`tester-14`/`tester-18` 通过抬高 A 的点击量把胜出方从 B 翻到 A（`推荐采用方案A` / `推荐采用标题A`），比只改数值更能守住「相对提升 + 胜出判定」整条链路。**工具升级**：弃用 `patch3.py`/`patch4.py` 的全数组重建，改用**精确 span 替换器 `patch5.py`**（只替换目标块的 `{…}` 字节区间 ⇒ 块间空行/注释/缩进逐字节不变；本批三文件实测「注释改动 0 / 空行改动 0 / 用例数不变」）。
  - ✅ **clicks 内改进程级全局对象必须用完即恢复（BATCH103 闭环，2026-09-25）**：`bonding` 3 例（`assessor-cycle-lifespan` / `detector-26` / `detector-27`）、`data` 3 例（`calc-2` / `random-6` / `random-7`）、`biz` 2 例（`unit-price-compare` / `assessor-risk-8`）去默认化（`all_default` 45→**37**、`no_inputs` 98 不变；判别器已检 2983→**2991** / 跳过 178→**170**）。**本批最重的新坑**：`random-6`/`random-7` 为把随机值钉死以便独立复算，在 `clicks` 里写 `Math.random=function(){return 0.123456;}` —— 但 **`Math` 是 Node 进程级全局对象**，`clicks`（页面作用域求值）改它不恢复会**污染同进程后续用例**：实测令 `random-7` 的**默认态**（已剥 `clicks`）仍走被钉死的 0.5 ⇒ 输出恒 `2025-07-01` ⇒ 默认态误 PASS（假逃生项，靠双态核验才抓到）。修法 = `var __r=Math.random;Math.random=fn;gen();Math.random=__r`。**随机页通用打法**：锚「三列连续复合串」（`12 0.1235 2111`）或「两行连续同值」（`1. 2025-07-01 2. 2025-07-01`），把巧合概率压到 10⁻⁶ 量级。**另三项锚点常识**：① 表头/范围说明常量（`1000-9999`、`2020-01-01`）、info-box 常驻文案（`建议结合实验验证`）、固定标签（`可疑信号面积比`）全是常量型逃生项；② `detector-26` 原 expect `建议返修补胶后复检` = 默认态 advice（该分支文案与缺陷态逐字相同）；③ `assessor-risk-8` 顶层 `var riskData` 用 `clicks` 覆写实现非默认，但**兜底阶段会无参调用 `removeRisk`/`addRisk`** 破坏它（删首项、加「新风险点」）⇒ expect 必须锚 `clicks` 阶段独有内容（阶段 2 判定早于该破坏）；④ `data/calc-2` 兜底阶段无参调用 `minifyJson` 会把 `result` 重写为压缩串 ⇒ 锚点须取 `formatJson` 的「冒号后带空格」形态。**零影响键**：`detector-27` 的 `material`/`adhesive` 未被 `calc()` 读取 ⇒ 不注入。
  - ✅ **同引擎多段渲染会共用常量串 + checkbox 默认缺陷项（BATCH104 闭环，2026-09-25）**：`edu2` 3 例（`exam-analysis` / `schedule-conflict` / `wrong-book`）、`floral` 2 例（`golden-ratio` / `spiral-bouquet`）、`ceramics` 1 例（`clay-shrinkage`）、`exhibition` 2 例（`assessor-60` / `assessor-evacuation`）去默认化（`all_default` 37→**29**、`no_inputs` 98 不变；判别器已检 2991→**2999** / 跳过 170→**162**）。**六种常量型逃生项形态**：info-box 常驻句尾（`月内推进签约转化`）、loadSample 兜底等级词（`不及格`）、结果区按钮文本（`复制清单`）、step-line 标签（`公式`）、静态 SVG 文本（`花器`）、下拉 option value（`pending`）与 checkbox 未勾的缺陷项（`应急广播缺失`）。**同引擎段污染（新形态）**：`exam-analysis` 的结果区在同一次 runCase 内被渲染两次（注入段 5 人 + 兜底 `loadSample()` 的 15 人样本），`40.0%` 在两段同时出现 ⇒ 弃用，改锚只属注入段的 `85.2 平均分` / `17.14 标准差` / `100.0% 及格率`。**sample 文本即逃生项**：`schedule-conflict` 原 expect `A101` 出自 loadSample 的示例教室 ⇒ 注入课程与样本用词完全错开（样本「高等数学/线性代数/英语」、注入「物理/化学/生物/历史」）且锚点带全角括号，避开 textarea value 被采集后的误命中。**日期相关量不可锚**：`wrong-book` 的 dueCount 与「剩 N 天」随运行日漂移（实测 837 天），只锚与日期无关的列表文本。**checkIds 反向利用**：`assessor-evacuation` 的 5 项消防 checkbox 桩内恒未勾 ⇒ 默认必出「xx缺失」并 score≥10，勾满后 score=0 ⇒ 锚「所有安全指标均满足规范要求」。
  - ✅ **同值巧合与「IIFE 封装 + 不可达分支」两类新形态（BATCH105 闭环，2026-09-25）**：`fun` 2 例（`bbq-portion` / `meditation-timer`）、`music` 2 例（`chord-notes` / `rhythm-trainer`）去默认化（`all_default` 29→**25**、`no_inputs` 98 不变；判别器已检 2999→**3003** / 跳过 162→**158**）。① **默认态同值巧合** —— `bbq-portion` 注入「14 人韩式含饮品」得 6300 g / 2800 g / 30 瓶，但主食 **2100 g 与默认态（6 人中式）的肉类总量逐字相同** ⇒ 若入断言则回退默认仍命中（真逃生项）；**定锚前必须把「候选锚点集合」与默认态全量 blob 逐串比对**，不能只看注入态输出。② **「不可达计算 + 可达派生量」** —— `rhythm-trainer` 全部函数声明在 IIFE 内，`clicks` 只能靠页面自身 `window.startTraining` / `window.adjustBPM` 导出调用（页面末尾显式挂 `window.*`，故全局可解析）；且 `startTraining()` 在 `renderGrid()` **之后**才因缺 `AudioContext` 抛错 ⇒ 评分/音符数/命中列表（`calculateScore` 及其后的一切）在 harness 内**永久不可达**，断言只能落在抛错点之前的产物（网格 8 个小节标签 `1 2 3 4 5 6 7 8`）与 `adjustBPM` 的 `Math.max(60,Math.min(200,…))` 上界夹取（140+75=215 ⇒ **200**）上。**同批另两例**：`chord-notes` 用 `analyzeChord()` + `F#m7` 锚三处结构串（`F#m7 · 小七和弦` / `C# 554.37 Hz 纯五` / `小七 (10半音)`，midi 66/69/73/76 ⇒ 369.99/440.00/554.37/659.26 Hz），旧 expect `261.63` 是默认输入 C 的根音频率；`meditation-timer` 改 35 分 / 6 段后**方式名一律不锚**（`10 分钟`「呼吸觉察」在结果区 7 天计划表内恒存在 ⇒ 常量型逃生项）。
...
- 存量弱用例 **123 例**（`no_inputs=98` / `all_default=25`），**转 P3 顺带**，不单独成批。
...
**存量 123 例**（`no_inputs=98` / `all_default=25`，selfcheck 口径；含 textarea/动态 id/结构性不可注入的「skip」类全站 158）。
...
**BATCH101 已处理（2026-09-25）**：`home` 5 例 + `hotel` 3 例 + `science` 4 例。**逐例打法**（注入态真实输出全部由 dump 探针取得，expect 均按标准公式独立复算）：① `home/furniture-layout` 走 `clicks` —— `rl=6/rw=5` + 加两件家具 ⇒ `360px|300px|床(双人)床(双人)餐桌床(双人)餐桌|none`（canvas 尺寸 = 边长×SCALE 60、家具名经 `appendChild` 反映进容器、有家具则 `empty-tip` 隐藏）；② `home/lighting-calculator` 走 `inputs`（length=6/width=4.5/lumens=1000/cu=0.7/mf=0.75）⇒ 面积 27㎡、客厅 100 lux ⇒ 总光通量 `27×100/(0.7×0.75)=5142.857` → **5143**；LED 8W `ceil(5142.857/800)=7` 只 ⇒ **7 只 LED 8W 合计 56 W**；③ `home/paint-calculator` 走 `inputs`（coats=3/coverage=12/bucket-size=4/bucket-price=300）⇒ 兜底清单总面积恒 52.0 ⇒ 总用量 `52×3/12=13.00` L、桶数 `ceil(13/4)=4`、成本 **1200**；④ `home/renovation-budget` 走 `clicks` + 覆写 `localStorage.getItem`（注入 8000×12 + 6000×10）⇒ **156,000**、项目数 2、均值 78,000、占比条 **瓷砖 61.5% ¥96,000**；⑤ `home/room-calculator`（**坏用例重写**）走 `clicks`（运行期给 `width=6/height=8` 赋值再 `calc()`）⇒ **48.00 / 28.00 / 10.00**；⑥ `hotel/currency-exchange` 走 `inputs`（fee=1.5）⇒ 手续费 `10000×1.5%=150`、净额 9850、到账 `9850×0.138=1359.3` ⇒ **150 手续费 / 1,359.3 USD 到账金额**；⑦ `hotel/itinerary-planner`（**坏用例重写**）走 `inputs`（4 天/9 小时/3 景点/2.5h）⇒ 总时长 **36h**、缓冲 **5.4h**、可玩 30.6h、优先级控件运行期生成读不到值回落 3 ⇒ 各景 `30.6×3/9=10.2h` ⇒ **10.2h 1.13天**；⑧ `hotel/occupancy-revpar` 走 `inputs`（100 间/售 70/收入 28000/1 天）⇒ **70% 出租率 OCC**、ADR `28000/70=400`、**¥280 每可售房收入 RevPAR**、评级「良好」；⑨⑩⑪ `science/newtons-second`（12/4 ⇒ **48.0 N**、**14.4000 加速度换算**）、`kinetic-energy`（12/4 ⇒ **96.0 J 动能**、**16.0000 速度平方**）、`pendulum-period`（2.5/9.8 ⇒ **3.173 s 周期**、**1.9799 角频率**）走 `inputs`；⑫ `science/molar-mass-calculator`（C6H12O6 ⇒ **180.156**、C 行占比 **40.00%**）走 `inputs`，**并同批修掉页面「占比」列真缺陷**（见 §7.1 / §十）。**本批未采纳的锚点**（默认态同样命中 ⇒ 零判别力）：`furniture-layout` 原「橱柜」（`#furn-list` 常量按钮列表）、`lighting-calculator` 原「合计」/`currency-exchange` 原「手续费」/`occupancy-revpar` 原「出租率」（均为 data-card 静态标签）、`paint-calculator` 原「客厅墙面」与 `renovation-budget` 原「暂无数据」（前者来自常量兜底清单、后者是空数据分支常量）、`itinerary-planner` 的「行程节奏合理」（默认 6/5=1.2 同样 ≤3 ⇒ 两态同句）、`occupancy-revpar` 的 `¥400`（ADR 默认与注入态同为 400）。

**BATCH102 已处理（2026-09-25）**：`misc2` 4 例 + `logistics2` 4 例 + `media` 3 例，全部属「inputs 逐字等于页面默认值」型 `all_default`。**逐例打法**（expect 均按页面公式独立复算，11 例注入态 `via=input event`、`inventory-aging` 为 `via=loadSample`）：① `misc2/car-residual` price=200000/years=5 ⇒ `RULE_RATES=[0.15,0.12,0.10,0.08,0.06]` 累计 0.51 ⇒ **51.00%** / **¥98,000.00**（默认 37.00% / ¥94,500.00）；② `misc2/cigarette-tar` 5mg/0.4mg/10 支 ⇒ **每日焦油摄入： 50.0 mg** / **估算吸入焦油（按25%吸收率）： 12.5 mg/日**；③ `misc2/screen-size` 720×1280@6.5" ⇒ **PPI 像素密度： 225.94** / **0.92 MP**（默认 392.81 ⇒「细腻」跨到「清晰」）；④ `misc2/tax-refund` amount=80000/rate=0.06（rate 是汇率）⇒ **实退金额： 7,600.00 JPY** / **折合人民币： ¥456.00**；⑤ `logistics2/inventory-aging` period=200 ⇒ **60.0 83 ¥12,500** / **16,000 ¥48,000**；⑥ `logistics2/loading-utilization` box 0.5×0.4×0.4/12kg、planQty=800 ⇒ **340 最大可装件数** / **170.5%**；⑦ `logistics2/packaging-cushion` 8kg/60cm/G40 ⇒ t = 3.2×60/40×(1+ln9×0.08) = 5.6437 ⇒ **5.6 cm 建议厚度** / **11.3 cm 总缓冲层厚度**；⑧ `logistics2/picking-route` 12 通道×25m/间距 4m/1.5m·s⁻¹/8s/4 点 ⇒ 344.0 m、344/1.5+48×8 = 613.3s ⇒ **总耗时 10 分 13 秒** / **24.6%**（默认 6 分 31 秒 / 25.4%）；⑨ `media/assessor-25` 8000/3200/2000/900/75s/2200 字 ⇒ **40.0% 打开率**（3200/8000） / **综合评分： 9/9**（三项均达标）；⑩ `media/tester-14` A 480/6000 vs B 360/6000 ⇒ **8.00% 方案A CTR** / **推荐采用方案A**（默认 B 胜 ⇒ 翻转）；⑪ `media/tester-18` A 250/3000 vs B 150/3000 ⇒ **8.33% 标题A CTR** / **推荐采用标题A**（默认 B 胜 ⇒ 翻转）。**本批未采纳的锚点**（默认态同样命中或两态同值）：`中焦油`（静态分级表）、`日元可退税`（国家表说明列）、`估算残值`/`建议装载`/`跌落下不受损`/`复制到后续内容`/`继续迭代测试`/`式更优`/`建议采用方案`（data-card 标签或结论句常量）、`inventory-aging` 的 `呆滞品数 4` 与 `77.0%`（period 无关，两态同值）。

**BATCH103 已处理（2026-09-25）**：`bonding` 3 例 + `data` 3 例 + `biz` 2 例，全部属「inputs 逐字等于页面默认值」型 `all_default`。**逐例打法**（注入态真实输出全部由 dump 探针取得，expect 均按页面公式独立复算）：① `bonding/assessor-cycle-lifespan` 走 `clicks:["calc()"]` + `inputs`（σb900/Δσ300/σm150/Kt1.5/设计80万次/表面0.8）⇒ σE=900×0.5×0.8/1.5=240.0、σEq=300/2/(1−150/900)=180.0、N=(240/180)^5×10⁶ ⇒ **421.4**（万次）、sf=421.4÷80 ⇒ **5.27**（默认 100.0 万次 / 1.00）；② `bonding/detector-26` 设计 0.3/公差 10/8 点 0.28–0.32/少量气泡 ⇒ avg **0.300**、极差 0.04 ⇒ cv **13.3%**、maxDev **6.7%**、测点 1 实测 **0.280 mm**；③ `bonding/detector-27` 回波 −2/衰减 5/面积 50/信号 6 ⇒ 落「需复检」（!attenOk）、面积比 6/max(5,1)×100 ⇒ **120.0%**、advice 含 **超声波信号异常**；④ `data/calc-2`（JSON 格式化）注入 `{"alpha":123,"beta":[1,2]}` ⇒ `JSON.stringify(data,null,2)` 输出含 **"alpha": 123** / **"beta": [**（默认空输入 ⇒「请输入 JSON 数据。」）；⑤ `data/random-6`（随机数字）cnt=8 + `clicks` 钉死 `Math.random=0.123456` ⇒ randInt(0,100)=**12**、toFixed(4)=**0.1235**、randInt(1000,9999)=**2111**，锚连续串 `12 0.1235 2111`；⑥ `data/random-7`（随机日期）cnt=8 + `Math.random=0.5` ⇒ `d=start+0.5×range`（范围 4017 天）⇒ 2025-07-01，锚连续两行 `1. 2025-07-01 2. 2025-07-01`；⑦ `biz/unit-price-compare` A 400g/10 元 + B 2kg/36 元 ⇒ **0.0250 元/g** / **0.0180 元/g** / 倍数 **1.39**（默认 0.0240/0.0200/1.20）；⑧ `biz/assessor-risk-8` `inputs{scene:production}` + `clicks` 覆写 `riskData`（5×4、4×4、2×2）⇒ maxRisk=20 ⇒ **极高风险-需立即处置** / 措施 **安全培训/设备点检/作业许可/防护装置/应急演练** / 行 **设备点检缺失**（默认 building + 默认三项 ⇒ 高风险-需优先整改）。**本批未采纳的锚点**：`建议结合实验验证`（info-box 常驻）、`可疑信号面积比`（固定标签）、`1000-9999`／`2020-01-01`（表头与范围说明常量）、`更划算`（两态都是 B 更划算 ⇒ 必须与具体单价绑成整段）、`建议返修补胶后复检`（默认态 advice）。

**BATCH104 已处理（2026-09-25）**：`edu2` 3 例 + `floral` 2 例 + `ceramics` 1 例 + `exhibition` 2 例，全部属「inputs 逐字等于页面默认值」型 `all_default`。**逐例打法**（注入态真实输出全部由 dump 探针取得，expect 均按页面公式独立复算）：① `edu2/exam-analysis` 满分 120 / 及格 50% / 优秀 80% + 5 人 108/96/90/72/60 ⇒ **85.2 平均分**、**17.14 标准差**、**100.0% 及格率**（默认 loadSample 15 人 ⇒ 74.9 / 14.24 / 80.0%）；② `edu2/schedule-conflict` 注入 4 条课程（物理·化学同教师同时间、物理·生物同教室同时间）⇒ **物理（D401） 化学（E502）**、**D401 物理（王老师） 生物（赵老师）**（默认样本 8 条零冲突）；③ `edu2/wrong-book` `clicks` 覆写顶层 `data`（pending / reviewing / mastered 各一）+ `filterStatus=pending` ⇒ **数学（1 题）**、**函数定义域求解**、**解析：x>0**；④ `floral/golden-ratio` `clicks` 切 `currentStyle='cascade'` + 花器 24 ⇒ **瀑布型 · 理想花艺高度 = 花器×1.25**、**花器 24cm**、**30 理想 (cm)**（默认 triangle ⇒ 22.5 cm / ×1.5）；⑤ `floral/spiral-bouquet` large/cascade/60/75 ⇒ **共需 35 枝花材**、**大型花束共35枝，瀑布型风格**、**主花 75%**（默认 18 枝 / 圆球型 / 60%）；⑥ `ceramics/clay-shrinkage` 150/135/120 + 反向 200/18 ⇒ **20.00%** / **10.00%** / **11.11%** / **243.90**（默认 6.67 / 10.71 / 16.67、116.28）；⑦ `exhibition/assessor-60` 15 万投入 + 60 万签约 ⇒ **4.00 ROI 投入产出比**、**单线索成本 ¥750**、**意向客户 90 45.0%**（默认 2.50 / ¥833 / 37.5%）；⑧ `exhibition/assessor-evacuation` 4000㎡ / 1500 人 / 6 出口 / 3.5m + `checkIds` 勾满 5 项 ⇒ **所有安全指标均满足规范要求**、**77.9 估算疏散时间(分)**、**0.38 人员密度(人/m²)**（默认 score=13 ⇒ 不合格 + 五项缺失）。**本批未采纳的锚点**：`不及格`（兜底等级词）、`月内推进签约转化`（info-box 常驻）、`复制清单`（按钮文本）、`公式`（step-line 标签）、`花器`（SVG 常量）、`pending`（option value）、`A101`（样本教室）、`应急广播缺失`（checkbox 默认缺陷项）、`40.0%`（同引擎两段都出现）。

**BATCH105 已处理（2026-09-25）**：`fun` 2 例 + `music` 2 例，全部属「inputs 逐字等于页面默认值」型 `all_default`。**逐例打法**（注入态真实输出全部由 dump 探针取得，expect 均按页面公式独立复算）：① `fun/bbq-portion` 14 人 / 韩式烤肉 / 含饮品 ⇒ 肉 450×14=**6300 g**、蔬菜 200×14=**2800 g**、主食 150×14=2100 g、饮品 `round(14×700/330)=round(29.70)=`**30 瓶**（默认 6 人中式含饮品 ⇒ 72 串 / 2100 g / 1200 g / 900 g / 13 瓶）；② `fun/meditation-timer` 35 分 / 每 6 分 / 咒语默念 ⇒ 分段 `Math.max(1,Math.round(35/6))=Math.round(5.833)=`**6 段**，锚 **35 分钟**、**间隔分段（每 6 分钟）**、**第 6 段**（默认 10 分 / 2 分 ⇒ 10 分钟 / 5 段 / 第 5 段）；③ `music/chord-notes` `inputs{chordInput:"F#m7"}` + `clicks:["analyzeChord()"]` ⇒ 根音索引 6（F#）、intervals [0,3,7,10] ⇒ midi 66/69/73/76 ⇒ 440×2^((n−69)/12) = 369.99 / 440.00 / **554.37** / **659.26** Hz，锚 **F#m7 · 小七和弦** / **C# 554.37 Hz 纯五** / **小七 (10半音)**（默认输入 C ⇒ 261.63 / 329.63 / 392.00）；④ `music/rhythm-trainer` `inputs{bpmInput:"140",measureCount:"8"}` + `clicks:["adjustBPM(75);startTraining()"]` ⇒ 140+75=215 被 `Math.max(60,Math.min(200,215))` 夹到 **200**（写回 bpmInput 的 value）、`renderGrid()` 按 `measures=8` 渲染 8 个小节标签 ⇒ 锚 **200** 与 **1 2 3 4 5 6 7 8**（默认 90+75=165、measureCount 回落首个 option=2 ⇒ 网格仅 `1 2`）。**本批未采纳的锚点**：`2100 g`（= 默认态肉类总量，同值巧合）、`—`（肉串数/饮品的空档位）、`咒语默念`/`10 分钟`/`呼吸觉察`（结果区 7 天计划表内恒存在）、`261.63`（默认输入 C 的根音频率）、`NaN`（兜底无参 `adjustBPM()` 把 bpmInput 写成 NaN 的副产物）、网格/评分中的 `总音符数` 与命中列表（`getAudioCtx()` 抛错之后不可达）。
...
| **「inputs 键是生成器模板串残留」的坏用例会同时骗过两把锁（2026-09-24 BATCH101）** | `home/room-calculator` 的 `${f}` / `${f==='radius'?3:5}_X`、`hotel/itinerary-planner` 的 `pri${i}`：① `selfcheck.weakKind` 见 `inputs` 非空、但 `_pageDefaults` 取不到该键 ⇒ `return null` ⇒ **不计入弱用例棘轮**；② `discriminate_check` 因 expect 永不命中而判「正确变红」。⇒ 该例既不在棘轮里、也不算逃生项，**静默失效**（与 BATCH99 的 `baking/mold-volume` 同形）。**巡检口径**：对 `scripts/verify_*_calc.js` 搜模板串残留键（形如 `${` 或 `+变量+`）；凡 `inputs` 键在页面静态 HTML 里 `grep` 不到 id 的，一律按坏用例重写 |
| **运行期生成控件的页面：注入 `inputs` 无效，必须走 `clicks`（2026-09-24 BATCH101）** | `home/room-calculator`（`shape-input`）、`hotel/itinerary-planner`（`spotList` 的 `spot{i}`/`pri{i}`）的控件由 JS 拼 `innerHTML` 生成 ⇒ 不在静态 HTML 内 ⇒ `pageDefaults` 取不到默认值、`runCase` 也注册不了该 id 的内联 `oninput` ⇒ 注入后 `calc()` 不会自动执行。**处置**：`clicks` 内 `document.getElementById(id).value=…` 再显式调 `calc()`（`clicks` 属「无 inputs 分支」的真实注入，判别器清空后必变红）。**副产物**：这些键不在静态 HTML ⇒ 该用例天然不进弱用例清单，正好靠 `clicks` 计入可检 |
| **canvas / style 型页面无文本输出（2026-09-24 BATCH101）** | `home/furniture-layout` 的「结果」是 canvas：尺寸只写进 `canvas.style.width/height`（`collectStrings` 不采集 `style`）、家具名只经 `createElement+appendChild` 反映进容器 `innerHTML`（容器自身可被采集）。**处置**：在 `clicks` 内把 `style.width`、容器 `innerHTML`、`empty-tip.style.display` 用竖线拼成一个复合串写入 `__p` 再断言（如 `360px｜300px｜…｜none`）。原 expect「橱柜」取自 `renderFurnList()` 的常量 `FURNITURE` 静态按钮列表 —— 与输入完全无关 |
| **localStorage 为唯一数据源 + fallback 为空数组 ⇒ 可在 `clicks` 内覆写 `getItem` 注入数据（2026-09-24 BATCH101）** | `home/renovation-budget`：`getItems()` 读 localStorage（桩 `getItem` 恒 null、fallback 为 `'[]'`），而 `setItems` 是 no-op ⇒ 直接调 `addItem()` 无法累积，输出恒空态。**新出路**：`clicks:["localStorage.getItem=function(){return JSON.stringify([…])};renderItems();calc()"]` —— 等价于「用户本就有数据」，可断言真实计算量（`156,000` / `78,000` / `瓷砖 61.5% ¥96,000`）；默认态 `getItem` 仍返回 null ⇒ 正确变红。**注**：`medical2/drug-expiry`、`travel/packing-list`、`cleaning/appliance-cycle` 当年按「结构性不可注入」保留，如需复评可复用此法 |
| **静态参考表 / 说明列里的档位词与结论句是常量型逃生项（BATCH102，2026-09-25）** | `misc2/cigarette-tar` 原 expect「中焦油」出自 `renderTable()` 渲染的**静态分级表**（≤5 极低 / 5–8 低 / 8–12 中 / >12 高 四档**默认态同样渲染**）⇒ 零判别力；`misc2/tax-refund`「日元可退税」是国家表的说明列；`car-residual`「估算残值」、`loading-utilization`「建议装载」、`packaging-cushion`「跌落下不受损」、`assessor-25`「复制到后续内容」、`tester-18`「继续迭代测试」均是 data-card 标签或结论句常量。 | 定 expect 前先 `grep -c` 该串在页面源码的出现次数，并确认它在**默认态 dump** 里是否也出现；档位词/结论文案只可与**具体数值**绑成连续串（如 `5.6 cm 建议厚度`）才可用。 |
| **同值陷阱：同一指标在两态数值相同 ⇒ 零判别力（BATCH102，2026-09-25）** | `logistics2/inventory-aging` 的「呆滞品数 4」「呆滞金额占比 77.0%（142,000/184,500）」由 `threshold`（180）决定，而本批只改了 `period`（90→200）⇒ 两态**完全相同**；同理 `misc2/tax-refund` 的起始购物额折合、`media/assessor-25` 的阅读完成度在改动键不同的情况下可能不动。 | 改完必跑**双态 dump 逐串比对**：只有「注入态有、默认态无」的串才能进 expect。另：`car-residual` 的 `life`/`salvage` 只在非默认折旧法下参与计算 ⇒ **零影响键写进 `inputs` 会造成「看似非默认」的假象**，应剔除。 |
| **clicks 内改进程级全局对象必须用完即恢复（BATCH103，2026-09-25）** | `data/random-6`/`random-7` 为让随机页可独立复算，在 `clicks` 内写 `Math.random=function(){return 0.123456;}` 把随机值钉死 —— `clicks` 是页面作用域求值，但 **`Math` 属 Node 进程级全局对象** ⇒ 不恢复会**污染同进程后续所有用例**：实测 `random-7` 的默认态（已剥 `clicks`）因此仍走被钉死的 0.5、输出恒 `2025-07-01`，默认态误 PASS（逃生项，靠双态核验才抓到）。 | 写成「用完即恢复」：`var __r=Math.random;Math.random=fn;gen();Math.random=__r`（本次 `gen()` 已完成、结果已落入 blob，恢复不影响判定）。**通用**：凡 `clicks` 内改 `Math`/`Date`/`Array.prototype` 等**进程级**对象，一律存原值并还原；只改页面自身顶层 `var`/`localStorage` 则无此问题（`runCase` 每次重建页面作用域）。 |
| **同引擎内「多段渲染」会让默认态与注入态共用同一批常量串（BATCH104，2026-09-25）** | `edu2/exam-analysis` 的结果区在**同一次 runCase 内被渲染两次**：注入段（我给的 5 人）与兜底段（无参 `loadSample()` 装入的 15 人样本）。两段的派生占比可能撞同一串 —— 实测 `40.0%` 在注入段（优秀 2/5）与默认段（60% 以下 6/15）**同时出现**，锚它等于零判别力。 | 定 expect 前先跑 dump 探针，**逐段**看清 blob 是哪几次渲染拼起来的；凡在两个及以上段落都出现的串一律弃用（本批改用只属注入段的 `85.2 平均分` / `17.14 标准差`）。**同源风险**：`schedule-conflict` 若注入课程与 loadSample 样本同名，textarea 的 value 会被采集 ⇒ 锚点必须与样本用词错开（本批靠「物理/化学/生物/历史」+ 全角括号格式差异）。 |
| **checkbox 在桩内恒未勾 ⇒「xx缺失」类缺陷项是默认态常量（BATCH104，2026-09-25）** | `exhibition/assessor-evacuation` 的 5 项消防设施 checkbox 在 harness 里默认 `checked=false` ⇒ 默认态必然 score≥10 并渲染「火灾报警缺失…应急广播缺失」（原 expect「应急广播缺失」正是这条，注入失败同样命中 ⇒ 逃生项）。 | 反过来用它做「合格」分支的强锚：用例加 `checkIds:[…5 项]` 勾满 ⇒ issues 空、score=0 ⇒ 锚「所有安全指标均满足规范要求」（默认态不可能出现）。同类页面（`safety/*`、`fire/*` 及所有带 `checkIds` 的评估页）可复用此双向打法。 |
...
| **「默认态同值巧合」：候选锚点可能与默认态某个输出逐字相同（BATCH105，2026-09-25）** | `fun/bbq-portion` 注入「14 人韩式含饮品」的输出是 `— 肉串数 / 6300 g 肉类总量 / 2800 g 蔬菜 / 2100 g 主食小食 / 30 瓶 饮品`，其中 **`2100 g` 与默认态（6 人中式）的肉类总量逐字相同** ⇒ 若把「主食 2100 g」写进 expect，判别器清空注入后仍会命中（真逃生项）。 | **定锚流程补一步**：把候选锚点集合与**默认态全量 blob** 逐串比对（不能只看「注入态与默认态整体是否不同」）。同源形态还有 `—`（肉串数/饮品的空档位，两态都有）。 |
| **IIFE 封装页 + 抛错点之后的代码在 harness 内不可达（BATCH105，2026-09-25）** | `music/rhythm-trainer` 全部函数声明在 `(function(){ … })()` 内，`fns[n]` 与 `typeof n==='function'` 均为空 ⇒ 只有页面自己 `window.startTraining = startTraining` 这类**显式导出**（本页导出 setDifficulty/adjustBPM/startTraining/stopTraining/onTap）才能被 `clicks` 调用；且 `startTraining()` 执行到 `const ctx = getAudioCtx()` 时因缺 `AudioContext` **抛错中断**，其后的评分/音符数/命中列表（`calculateScore` 及其后）**永久不可达**。 | 断言只能落在**抛错点之前**已渲染的产物（本页 = `renderGrid()` 的小节标签 `1 2 3 4 5 6 7 8`）与**独立可调函数**（`adjustBPM` 的 `Math.max(60,Math.min(200,…))` 夹取：140+75=215 ⇒ 200）上。**判据**：先在 `clicks` 里试调目标函数，看 `errs` 是否报 `… is not a constructor` / `… is not defined`；报错即说明该函数之后的分支不可断言。 |
