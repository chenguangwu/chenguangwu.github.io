...
> - **遗留**：① 5 条无源键（页面有深度块但 JSON 无源）：`automotive/auto-beauty-calc-1`、`electrical/power-factor-compensation`、`photo2/white-balance`、`stage/stage-color-filter`、`wedding/wedding-budget-planner`；② 质量类缺口（不计入结构达标）：**664 页 examples 无真实算例**、184 页 examples 标题套话、9 页英文名嵌入中文。提交 `61d0516d8`。
