...
- [ ] **跨分类真重复工具去重（D 全站审计，2026-09-17 启动）**：全站 h1 归一化聚类得 25 组同名（50 页）。首批已下架 5 个确认真重复（唯一命名、不跨分类复用）：`sports/diving-no-decompression`、`sports/time-30`、`tcm-pharmacy/generator-28`、`it/mime-type-lookup`、`it/http-methods-reference`（commit `9bffc3229`，214 门禁全过、线上删除页 404、tools.json 残留 0、总数 4821→4816）。**待处理**：① 同名异功能 2 组（判定不下架，仅记录）：`finance/salary-after-tax`(累计预扣)vs`payroll-calculator`(含年终奖比例)、`ophthalmology/self-assess-2`vs`osdi-scale`(OSDI 两算法差异)；② `agriculture/calc-2` 因 slug 在 30+ 分类全局复用、guides.json 按文件名注册，**移出本批单列专项**（涉及"全局 calc-N 编号页"系统性命名问题，动一处会牵连多行业）；③ 25 组之外还有大量**跨分类标题近似**（非完全同名）未排查，需后续做模糊相似度聚类。脚本 `scripts/retire_crosscat_dupes.py`（幂等）。
