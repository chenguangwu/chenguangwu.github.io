...
5. **下拉选项只是占位或不合理**：选项要真实、合理、有业务意义。（§4.1.3）
6. **结果正确性未验证**：需验证工具输出结果正确。（§4.1.1）
7. **专业名称缺外链**（§4.1.7）：**已调研，受阻**——百度百科对非浏览器请求一律返回 403（含不存在词条），**无法本地验证词条真实性**，批量加外链的死链风险不可自查。替代方案待老板定：① **站内内链**（工具 deep-dive 的专业术语 → 站内对应工具页/指南页；零死链、可校验、内链 SEO 收益确定，推荐）；② 克制版百科外链（仅少量高置信术语、每页 ≤2，需承担死链风险）；③ 不做。
...
- **隔离器（`/tmp/audit_finance_all.js`）桩缺口两项，未修**：① `document.querySelector('input[name=x]:checked')` 恒返回 null → 单选/复选门控页输出 `undefined`/`NaN`（`optical/progressive-corridor` 等）；② `innerHTML=` 动态生成的控件（`fim-scale`/`womac`/`gingival-index`/`load-curve` 等，静态 HTML `grep id=` 为 0）取不到值。修法：`:checked` 查询返回桩元素且 `checked` 可注入；`innerHTML` 赋值时解析 `id=` 并注册桩。**未改前，凡默认输出出现 `undefined`/`NaN` 一律先按桩盲区排除，再回源码复核。**
  - **2026-09-23 实测补记（重要，勿再盲改 harness）**：对 ② 在正式门禁 `scripts/verify_it_calc.js` 实施「`innerHTML` setter 解析 `id=` 注册桩元素」改造（module 级 `_dynEls`/`registerInnerHTMLIds`，每个用例 `_dynEls=elements` 指向当前表）。全量 `run_gates --skip-build` 立即暴露 `psychology/calc-12`（幸福感量表）抛 `Cannot read properties of undefined (reading 'length'/'trim')`——该页用 `innerHTML` 渲染滑块，`getElementById` 读到空桩后读 `.length`/嵌套属性即崩。**根因**：harness 对 `innerHTML` 动态生成控件的**真实 DOM 行为**模拟缺失（不止取 id，还需像真实 DOM 一样被读取/操作、有真实值），单点 `id` 注册会误伤**所有用 innerHTML 渲染 UI 的页**，且多数盲区页无 verify 用例覆盖、门禁抓不到 → 真实误伤面更大。已 `git checkout HEAD -- scripts/verify_it_calc.js` 还原，216/216 恢复全绿。**结论**：② 不能靠 `id` 注册单点解决；真正闭环盲区页须立项「innerHTML 真实 DOM 模拟」专项（大工程，需评估工期），在此之前**维持原纪律"盲区先排除，再回源码复核"**。缺口①（`:checked` 默认返回桩）同理风险更高（影响所有单选/复选门控页），未实施。
...
> - **BATCH52（缺陷 M 收尾补充批，2026-09-22，✅ 5/5 页升 A）**：全站只读诊断扫描发现原 32 页扫描漏掉的 **5 个跨行业「通用两参共享脚本」名不符实残页**（标题未命中任一分支正则、直接落兜底通用二元计算）：`textile/fuliao-lalian-niukou-guige`（拉链/纽扣规格）、`aerospace/length-temp-runway`（ISA 跑道长度温度修正）、`aerospace/huoyun-uld-jizhuangqi-guige`（ULD 装载率）、`chemical/calc-pipeline-pressure-drop`（达西–魏斯巴赫压降）、`chemical/miaomu-guige-zhiliang-yanshou-biaozhun`（苗木验收判定）。手法同批次 1：页内重写独占 `calc()`/`resetAll()` + 真实多输入 + `formula-eq`/`info-box`（保留 URL/文件名），5 页由 B 升 A；B 级 37→32、C 级 4。3 处旧 verify 用例（锚通用输出 `7500.00`/`暂无计算记录`）改非默认输入 + 逐字复算，并排除 2 个逃生项（`湍流` 默认输出亦含、`合格` 在 info-box 静态参考文本）—— 门禁 215/215、判别力 0 逃生。脚本 `/tmp/rework_defect_m_b52.py`。
> - 剩余候选：**缺陷 M 全部 37 页闭环**（BATCH47–51 共 32 + BATCH52 补充 5）；own_len 300-699 波段已清零；缺陷 J 14/14、缺陷 L 4/4、缺陷 M 存根批 + 重做批 + 补充批 **全部闭环**。仅剩 4 个 C 级（2 个 AI 页属度量盲区 + 2 个静态展示页，天生 C 级，不宜硬凑）—— 缺陷 M 主线收口完成。
...
- **用法**：`/tmp/dump_all.py`（40 页一块 + 块超时自动细分到单页）产出 `/tmp/all_text.txt`（每行 `@@@ <slug> TEXT: <值 标签 值 标签…>`）；再用 Python 正则做**值→标签紧邻配对**筛查越界（概率/p 值 ∉[0,1]、率类 >100%）。
...
- **缺陷 D（已闭环，2026-09-22）** `hr/tracking-hours` + 四例「通用双输入」页：① `deadline`/`dept` 两个输入未参与计算（装饰性伪输入）；② `bandwidth-1`/`calc-81`/`eap-xinli-zixun-weiji-ziyuan`/`hris-zizhuyuaiduibijisuanqi`/`hr/performance-ranking` 共用「按 h1 正则选计算模式」模板，标题未命中 → 落兜底通用二元计算。6 页全部已修：**bandwidth-1 / calc-81 / performance-ranking** 在 BATCH46（缺陷 M）改页内独占 `calc()`（薪酬带宽 / 内推激励 / 绩效归一化）；**eap / hris** 在 BATCH50 重做（EAP 方案分级 / HRIS×AI 成熟度权衡）；**tracking-hours** 2026-09-22 修：让 `deadline` 驱动「剩余天数 + 建议周补训学时」追赶计划、`dept` 作为统计范围进入 summary 与提示文案，消除两个伪输入。门禁 215/215、判别力 0 逃生。**缺陷 D 全部 6 页闭环。**
...
- **缺陷 Q（2026-09-22 发现并闭环）`design/detector-29` 空分组导致 NaN% 与合规误判**：`a11yItems` 14 项只含 `A`(10)/`AA`(4) 两级、**无 `AAA` 项** → `levelAAA_total` 恒 0 → ① `Math.round(0/0*100)` 恒显示 **`NaN%`**；② `levelAAA_pass===levelAAA_total`（0===0）恒真 → 表格误显「✓ 通过」、`conformanceLevel` 误判「AAA级合规」。修：分组通过率 `total=0` 时显示「—」，A/AA/AAA 三行与合规等级判定一律加 `total>0` 前置。**通用形态**：凡「分组计数相除 + 相等即通过」的评分页，**空分组会同时产生 NaN 与假合规**，普查时应重点看 `0/0` 与 `pass===total` 组合。该页无 verify 用例 → 补一条覆盖核心对比度计算（黑白 `21.00:1`，W3C 公式）。
- **缺陷 R（2026-09-22 发现并闭环）`statistics/one-sample-z-test` 双侧 p 多乘 2**：`p=2*(1-erf(|z|/√2))`，但 `erf(x)=2Φ(x√2)−1`，正确为 `2(1−Φ(|z|))=1−erf(|z|/√2)` → 结果**恒为正确值的 2 倍**：z=2 得 `0.091`（应 `0.0455`）、z=1.96 得 `0.10`（应 `0.05`），**把本应拒绝 H₀ 的检验判为不显著**。页面 `formula-eq` 与「计算说明」也锚在错值 `0.091` 上，同步修正。全站 audit：`2*(1-erf(` 仅此一处，其余 20 处 `0.5*(1+erf(...))=Φ` 均正确。该页无 verify 用例 → 补一条（z=1.5 → p=0.1336）。
- **缺陷 S（2026-09-22 发现并闭环）`statistics/f-test-variance` deep-dive 算例口径撞车**：输入标签为「样本1方差 s₁²」（既有 verify 用例 `s1=36/s2=9 → F=4` 亦按方差），但 deep-dive 算例把同名符号当**标准差**（`F=144/100=1.44`）→ 用户照算例输入会得到错误结果。统一为方差口径（`F = 12/10 = 1.2`）。
- **缺陷 T（2026-09-22 发现并闭环）`science/p-value-calculator` 双侧 p 公式错误（与缺陷 O 同源）**：`p=2*(1-Math.abs(cdf-0.5))`，z 检验与 t 检验两分支各一处 → z=1.96 得 **1.05**、z=2.5 得 **1.0124**（均 >1）且 `p<α` 恒失败 → 结论反转（z=2.5 正确 p=0.0124 应拒绝）。修 `p=2*Math.min(cdf,1-cdf)`。**发现途径**：全站默认输出扫描（见 §8.8）。
- **缺陷 U（2026-09-22 发现并闭环）卡方 CDF 的 Wilson–Hilferty 变换括号缺失**：`science/chi-square-calculator` 与 `science/p-value-calculator` 两份副本写作 `z=(x/df)^(1/3)-(1-h)/sqrt(h)`，正确为 `[(x/df)^(1/3)-(1-h)]/sqrt(h)`（**均值项也要除以 √h**）。后果远超「近似误差」：`χ²=9.49,df=4` 得 **p=0.996**（真值 0.050）、`χ²=12.5,df=4` 得 0.994（真值 0.014）→ **高度显著被判为不显著**。改用精确解：正则化不完全 Gamma `P(a,x)`（`x<a+1` 级数，否则连分数，精度 ~1e-15），实测 7 组值与 Python 精确解逐位一致。补 verify 用例（`O=[35,25,25,15]`/E=25 → χ²=8、df=3、p=0.046）。
...
- **候选**：386 页（有 `formula-eq` + 真实 `calc()` + 数字输入，可被注入复算），覆盖 10 高热度行业；`finance` 40+ 页全未覆盖（Y，优先）。`science`（99 页）已在 BATCH47–49 走过一遍默认态复算（隔离器升级后 78 页可读）；`math`（36 页）已在 BATCH50 全量复算并闭环 14 页缺陷；`geometry`（28 页）已在 BATCH51 全量复算并闭环 15 页缺陷；`photo`（31 页）已在 BATCH52 全量复算并闭环 5 页缺陷；`ai`（56 页）已在 BATCH53 全量复算并闭环（1 处默认态语义缺陷 + 31 页边界守卫 + 2 处深度解析算例订正）；`sports`（56 页）已在 BATCH54 全量复算并闭环（5 处默认态确凿缺陷 + 8 例新增/1 例改写用例）；`agriculture`（52 页）已在 BATCH55 全量复算（默认态无确凿数值缺陷，边界 3 页零值 NaN 守卫 + 5 例用例）；`finance`(113) 已在 BATCH56 全量复算并闭环（1 处默认态确凿缺陷：tax-calculator 税率级距下限硬编码成 3,001 + 1 例用例）。截至 BATCH56，方向1 候选 10 行业（science/math/geometry/photo/ai/sports/agriculture/finance + 首批 finance 残页）**全量闭环**。
- **金融周边集群延伸（BATCH57 + 顺带核验，2026-09-23）**：老板选定方向1 主线（找错钱数/错物理量）延伸至金融周边高流量计算行业。banking(26)/investment(24)/tax(29)/realestate(53)/accounting(36)/insurance(34)/economics(28)/statistics(52)/forex(6)/futures(6) 共 10 行业 ~334 页隔离器全量扫描：仅 **securities/calc-30 一处确凿缺陷**（默认 freq=252 使 13 点月度样本年化 CAGR≈67,919% 荒谬，改默认月(12)→36.43%，BATCH57 已修复+1 例用例+门禁 217/217）。其余 NEG 页（effective-annual-rate/fisher-real-rate 之「精确−近似」负差、analysis-46 残差表、assessor-risk-11 标准 Altman Z、insurance 赔付差/对数生存率、economics 税收乘数、statistics IQR 下界/logit、bond-convexity 利率上行价跌、calc-29 Bollinger 下轨、assessor-return 空置损失减法、rent-1 A−B 差值）经独立复算均正确，NEG 为启发式误报（科学计数法 e-6 / 内部减法 / 合法负财务指标）；forex/pip-calc、forex/spread-cost（`pip` undefined）、futures/option-payoff（`ctx.rotate`）为隔离器盲区（select option dataset / canvas 2d 桩未建模），真机正常，排除不立项。finance 集群（banking→futures）**全量健康，方向1 主线至此全量闭环**。
- **中低热度延伸（BATCH58，2026-09-23，仅本地提交）**：老板选方向2「向中低热度行业延伸」。隔离器全量扫描 113 个待精查行业（页≥15），结论：**默认态全健康**（所有 ERR/NaN 页经分类均为隔离器盲区：NO_TRIGGER / document.getElementsByName is not a function / location is not defined / ctx.rotate / 桩缺变量等，真机正常，与 forex/futures 的 pip/ctx.rotate 盲区同类）。边界探针（ZERO/EMPTY 置 0/空）抓出物理工程类 ~150 页级 NaN 健壮性缺陷（用户输入 0/空时未守卫渲染 NaN），与 agriculture BATCH55 同类。BATCH58 以 **thermodynamics 全 27 页** 作代表批：批量注入「输出检测守卫」（dataGrid 含 NaN/Infinity → 渲染告警而非崩溃），探针 FLAGGED 27→0；新增 verify 边界用例 1（boyles-law 输入 0 → 期望「无效值」），verify 28/28、判别力 0 逃生、门禁 217/217；基线 total_cases 3159→3160、checked 2631→2632。其余行业（fluid/electromagnetism/optics/acoustics/hydraulic/structural 等）边界 NaN 记录为待办（P3 顺带或下批）。
- **执行注记（2026-09-23 收尾）**：本批实际仅把 **thermodynamics 27 页守卫本地提交**（`tools/thermodynamics/` 27 文件；boyles-law 边界用例已在 HEAD 的 `verify_thermodynamics_calc.js`）。**全站 4525 文件「一键注入」曾跑过但已 `git checkout HEAD -- tools/` 回退**——注入工具 `scripts/_inject_output_guard_v7.py` 对含**模板字符串（`${...}`）或动态结构**的 `innerHTML` 赋值做守卫包裹时会**搞坏页面**：实测 `tcm-chemistry/response-factor` 出现 `${d.name}` 字面量、`rehabilitation/asia-impairment-scale`/`dentistry/gingival-index` 输出只剩 UI 模板（期望值消失），且 `food/wine-alcohol-converter` 守卫在 headless harness 下对**正确输出误触发 ⚠**（动态 select 默认空值→`parseInt("")=NaN`→字面 "NaN" 被守卫命中）。**全站边界 NaN 闭环须先硬化注入工具（模板字面量感知扫描 + 排除 harness 不可驱动的页面），列为下批/顺带 TODO，不与本批同交**。注：本批（含续作）`run_gates.py --skip-build` 实测 **216/216 全绿**（反回归 0 逃生项）。续作（commit 17b94e9c36，仅本地、不 push）已把其余 7 物理工程行业（fluid 23/electromagnetism 26/optics 21/acoustics 28/hydraulic 56/structural 26/mechanical 14，共 194 页）守卫本地提交，每行业注入后即跑该行业 verify 全过、全量门禁 216/216。**仍不碰含模板字面量/动态结构的非物理页**（dentistry/food/rehabilitation/tcm-chemistry 等），全站闭环**已通过硬化注入工具实现**（模板字面量感知扫描 + 排除 harness 不可驱动页）：本次硬化 `scripts/_inject_output_guard_v7.py`（新增 `expr_is_safe_to_guard` 保守过滤，只注入纯变量/纯数值 RHS，跳过模板字面量/字符串/方法链/DOM 渲染函数；加 `--dry`；已注入页绝不重触）+ 全站铺开——208 个有 verify 覆盖行业中 182 个新增注入 **3421 页**（`run_gates --skip-build` 216/216 全绿，反回归 0 逃生项）；4 个坏分类（dentistry/food/rehabilitation/tcm-chemistry）因含 harness 盲区页（动态 select 默认空值→数值 NaN 误触发）整类回退 HEAD 零破坏。**基线纠正**：HEAD 实际仅 1034 页已注入守卫（之前「全站已注入」为误判），本次后全站可安全注入页（静态可驱动 + 纯数值 RHS）全量闭环；动态 select 默认空值类 harness 盲区页留待硬化 v8 专项（识别并跳过 harness 不可驱动页）。
