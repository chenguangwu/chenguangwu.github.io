...
> - **✅ 目标已达成并超额，bucket1 候选池已清零**：A 级率 70.0%→**83.1%**（3339→3959），共 19 批 620 个工具升级（bucket1 = `own_len≥800 且非 rich` 全量消化完毕）。后续若需再提 A 级率，只能转向 bucket3（`own_len 与 inputs 双不足`）等更重路径，须另立专项评估。
