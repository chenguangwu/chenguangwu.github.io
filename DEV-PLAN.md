...
### ✅ agriculture（65 键，完整分类收口，最新一条）
- 65 键中 60 键已有对应 HTML 且已真实化（更早批次完成）；本轮仅补齐剩余 5 个**孤儿占位键**（calc-3 连作障碍指数 / calc-4 大棚卷膜时间 / calc-5 温室通风量 / calc-11 农机具作业效率对比 / calc-14 作物需水量），将其按标题对应的标准农艺公式真实化（3 场景+1 算例+2 FAQ）。
- 关键发现：calc-3/4/5/11/14 在 content_deepdive.json 中有键与标题，但 tools/agriculture/ 下**无对应 HTML 页面**（孤儿键）。按项目铁律（键数守恒 5022、不擅删任务项），不删键，按领域公式真实化；无源页面故未读 calc()。算例数字 node 实跑复核（/tmp/verify_agri.py）：calc-14 ETo5.0×Kc1.05=5.25mm/d、2ha→105m³/d；calc-5 30×8×3.5=840m³、ach30→25200m³/h(ach40→33600)；calc-4 60m÷0.8m/s=75s≈1.25min；calc-11 20亩 A2.5h/B2.0h→8.0/10.0亩/h、B快25%；calc-3 (5−1)×15+60×0.5=90 重度。
- 全部 65 个 agriculture 键经六型指纹(SIX+GEN)扫描 = 0 占位残留；五道门禁全过（build+静态+死链+资产+公式回归 ALL OK）；键数守恒 5022。脚本：scripts/apply_agriculture.py。
- 待决：5 个孤儿键无对应页面，真实内容仅存于 JSON（无页面渲染）；建议后续由老板决定是否补建 5 个工具页或清理孤儿键（涉及键数 5022 不变量，需先确认）。

## 八、当前进行中分类

> 当前无进行中分类。新分类开工时先在此登记（目录 + 工具数），收口后写入第七节并覆盖旧归档记录（§4.3 第 4 步）。

## 九、分类总清单（待办，完成一个删一个；剩 47 个目录）
