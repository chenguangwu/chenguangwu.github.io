...
### ✅ tunnel（5 键，真实化收口，最新一条）
- 5 个工具全部真实化（lining-thickness 二衬 h₂=k×跨度B 系数表 0.04/0.05/0.06/0.075/0.09/0.10 + 初衬定厚 5/8/12/15/20/25cm + ≥Ⅳ级配筋 + 每延米砼量≈h₂×2×(B+5) / advance-rate 循环进尺=孔深×利用率、循环时间=钻+爆+通风+出渣+支护、循环数=min(计划, floor(24÷循环时间))、日/月进尺与1000m工期 / support-design 锚杆长=系数(0.20~0.45)×跨度且≥2m、间距=min(L/2, 级别上限1.5/1.2/1.0)、喷层5~25cm、每延米根数=ceil(周长÷S²) / ventilation-calc 风速法 Q=A×v×3600 与 CO 法 Q=q×N×L÷δ 双模式 + 换气次数 + 风机选型 / gas-monitor 瓦斯五级分级 0.5/0.8/1.0/1.5% + 断电阈值1.0% + 爆炸区间5%~16% 判定），每个含 3 真实场景 + 1 算例 + 2 专业 FAQ。
- 算例数字全部 node 实跑复核（/tmp/verify_tunnel.js）：lining B=10 → Ⅰ级 h₂0.4m/总45cm/12.0m³、Ⅱ级0.5/58/15.0、Ⅲ级0.6/72/18.0、Ⅳ级0.8/95/24.0需配筋、Ⅴ级0.9/110/27.0、Ⅵ级1.0/125/30.0，Ⅳ级B=14→h₂1.1m/总125cm/41.8m³；advance 3m/0.9/钻3爆1通风0.5出渣3支护2/计划3 → 进尺2.70m、循环9.50h、时间允许2次、实际2次、日5.40m、月162m、工期6.2月；4m/钻4 → 3.60m/10.50h/2次/7.20m/216m/4.6月；2m/0.85 快循环 → 1.70m/6.50h/3次/5.10m/153m/6.5月；support 12×8 → Ⅰ级 L2.4/S1.2/5cm/28根/67.2m、Ⅱ级3.0/1.5/8/18/54.0、Ⅲ级3.6/1.2/12/28/100.8、Ⅳ级4.2/1.0/18/40/168.0、Ⅴ级4.8/1.0/22/40/192.0、Ⅵ级5.4/1.0/25/40/216.0；vent 60m²/2.5 → Q540000m³/h=150.00m³/s、换气90.00次/h、单机75.00m³/s×2，80m²/2.0 → 576000/160.00/72.00/80.00；gas 0.30安全·0.50注意·0.65注意·0.90警戒·1.20危险断电·2.00极度危险·8.00爆炸区间·20.00超上限缺氧。
- **量纲如实标注**：ventilation-calc 的 CO 模式按 Q=q×N×L(m)÷δ(ppm数值) 直代，默认输入输出 160 m³/h（0.04 m³/s）；按标准口径（L 取 km、δ 取 1e-4 体积比）应为 160000 m³/h≈44.4 m³/s，deep-dive 已在 FAQ 如实注明二者差异与正确换算，不掩盖工具行为。换气次数按 v×3600÷100（以 100 m 洞段为基准）亦已注明。
- lining-thickness 含旧版 opt-guide/opt-faq 套话块(2块)+FAQPage LD 旧套话，经 opt_faq_ld_sync(--cat tunnel 同步2条)+opt_cleanup_opt_blocks(--cat tunnel 删2块)清零（LD JSON 校验合法）；其余 4 文件原 LD 干净。五道门禁全过（含清理后复跑）；六型指纹(含"先统一"变体)+可见 opt 块扫描 0 残留；真实关键词 lining2/advance3/support3/vent8/gas4；键数守恒 5022。脚本：scripts/apply_tunnel.py。

## 八、当前进行中分类

> 当前无进行中分类。新分类开工时先在此登记（目录 + 工具数），收口后写入第七节并覆盖旧归档记录（§4.3 第 4 步）。

## 九、分类总清单（待办，完成一个删一个；剩 34 个目录）
