...
- `all_default 91 / no_inputs 107 / escape 0`；`checked=3161`；门禁 `run_gates.py` **217 项全过**、逃生项 0（判别器已检 **2928** 例 / 跳过 233）。
- A 级率 **99.2%**（A 4693 / B 32 / C 4，共 4729）；deep-dive 术语内链 **1591 页 / 2268 条 / 唯一目标 630**（零死链、零自链、单页 ≤6 条）。**注：内链不影响质量分级**（`own_len` 只统计 `<script>` 内容）。
- 存量弱用例 **198 例**（`no_inputs=107` / `all_default=91`），**转 P3 顺带**，不单独成批。
...
**存量 198 例**（`no_inputs=107` / `all_default=91`，selfcheck 口径；含 textarea/动态 id/结构性不可注入的「skip」类全站 233）。
