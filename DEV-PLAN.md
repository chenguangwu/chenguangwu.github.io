...
- [ ] **`energy` 分类内重复工具组待治理 —— 2026-09-17 已取证（待确认 301 规划，未删）**：逐页比对 calc 公式与输入输出，结论：① 热泵×3（`heat-pump-cop`/`cop-heatpump`/`calculator-calc-5`）**确认真重复**——同算 COP，`heat-pump-cop` 最全（含 EER+运行电费），另两页为其子集；② 光伏×3 中 `solar-output-physics` 与 `solar-panel-power` **近重复**（同公式 P=A·G·η，仅输入命名/口径微异），`solar-calculator` 为不同层级（年发电量+电费估算）；③ `energy-density` 与 `specific-energy` **同义真重复**（均 E/m，中文名不同"能量密度/比能量"）。处置需下架冗余页并 301 指向保留页（影响 SEO），按"风险操作前推回"原则，待老板确认合并/下架 + 301 方案后再动，未擅自删。
- [x] **`finance` 分类混入非金融工具 —— 2026-09-17 已完成（零 301）**：核实 11 个嫌疑工具，仅 8 个确为非金融混入（word-scramble / wifi-password-show 在 finance 目录不存在已剔除；currency-converter 货币换算确属金融剔除）。**修法：仅改页面 meta `industry=`（分类页按 industry 分组、URL 按目录，故零 301）**，目标行业均核存在于 INDUSTRY_DEFS（注意 `dev` 不存在，密码/二维码改归 security/general）：driver-license-validator→life、mirror-text→text、word-wrap→text、word-search→fun、dns-record-info→network、password→security、password-generator-advanced→security、vcard-qr→general。构建重建触发 141 个相关卡 rt-icon + 分类索引/industry-json/sitemap/tools.json 结构性更新（无内容破坏，214 门禁含链接/资产审计全过已证）。脚本 `scripts/fix_finance_misclass.py`（幂等）。提交待推送。
