...
**处置**：属新线（补用例 ≠ 改弱用例），单独立批；须守 §8.1（expect 独立复算）。**已交付 44 例**（`design/*` 8 + `edu/*` 5 + `biz/*` 31，逐例锚点见各用例 `ref`）。剩余 **170** 页待补（`design/*` 89、`edu/*` 36、`biz/*` 45）。【按 `tools/**` 真实文件数重校】`design/image-resizer`（`generate()` 首行 `if(!origImg) return` + 依赖 canvas 解码）、`edu/exam-study-planner`（localStorage 桩只写不读 ⇒ 统计分子/分母不可达）**结构性不可注入，不硬写用例**。
...
- `all_default 4 / no_inputs 10 / escape 0`；门禁 `run_gates.py` **216 项全过**、逃生项 0（判别器已检 **3153** 例 / 跳过 50）。
