...
- [ ] **跨分类真重复工具去重（D 全站审计，2026-09-17 启动，进行中）**：
  ...
  - **剩余（待续）**：108 对中的其余 ~87 对需更细 canonical 规则——含全局复用编号页（calc-N/rater-N 跨行业同名，guides.json 按文件名注册）、描述性短名对（`date-diff` vs `date-difference-calculator`、`ev` vs `exposure-value`、`calc-volume` vs `prostate-volume`、拼音 vs `womac` 等）、以及 156 对跨目录相似（可能同名异功能，需逐对语义判定）。
