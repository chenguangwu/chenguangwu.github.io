...
- **存量分批专项（进行中）**：改 expect 若取自页面自身输出即"自证循环"，正是 10.1 的假门禁老路；正确修法须**逐例按标准公式独立复算**，故分批推进。
  ...
  - **剩余 466 例**（`no_inputs=212` / `all_default=254`）待后续批次，按"公式可可靠独立复算 + 页面存在可注入控件"两条同时满足的分类依次推进。**选批前必做可注入性预筛**（第十四批教训：psychiatry 22 例里 18 例是纯点击答题页，无可注入控件，白跑一半）。截至第十八批的可注入性实测（弱用例数 / 页面含静态表单控件数）：dyeing 12/12、dermatology 14/11、engineering 14/11、signal 11/11、design 10/10、rheumatology 14/9、endocrinology 10/9、mining 10/9、gas 9/9、mechanical 9/9、travel 9/7、gardening 8/7、finance 7/7、sports 7/7、fire 7/6、chemical 9/5、cleaning 7/5 —— 下批建议取 **dyeing 12**（`ecommerce 17` 已于第十六批收口、`pulmonology 14` 已于第十七批收口、`advertising 12` 已于第十八批收口；实测 advertising 收口 16 例：12 all_default + 2 例 textarea 回显型 + 2 例依赖兜底函数的用例）。
