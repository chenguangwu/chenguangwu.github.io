...
- **存量分批专项（进行中）**：改 expect 若取自页面自身输出即"自证循环"，正是 10.1 的假门禁老路；正确修法须**逐例按标准公式独立复算**，故分批推进。
  ...
  - **剩余 454 例**（`no_inputs=212` / `all_default=242`）待后续批次，按"公式可可靠独立复算 + 页面存在可注入控件"两条同时满足的分类依次推进。**选批前必做可注入性预筛**（第十四批教训：psychiatry 22 例里 18 例是纯点击答题页，无可注入控件，白跑一半）。截至第十九批的可注入性实测（弱用例数 / 页面含静态表单控件数）：dermatology 14/11、engineering 14/11、signal 11/11、design 10/10、rheumatology 14/9、endocrinology 10/9、mining 10/9、gas 9/9、mechanical 9/9、travel 9/7、gardening 8/7、finance 7/7、sports 7/7、fire 7/6、chemical 9/5、cleaning 7/5 —— 下批建议取 **dermatology 14**（`ecommerce 17` 第十六批、`pulmonology 14` 第十七批、`advertising 12` 第十八批、`dyeing 12` 第十九批均已收口；实测 dyeing 收口 13 例：12 all_default + 1 例 select 值回显，其中 **11 例 expect 是 historyBox 的「暂无计算记录」常量**）。
...
- **缺陷 H（占位页「名不符实」，第十一批缺陷 F 的同类复发，第十八批查出）`advertising/analysis-27`、`advertising/analysis-55`**：两页标题分别为「竞品（份额/定位）分析」「竞品（广告/投放/创意）分析」，但**共用同一段与标题毫无关系的通用描述统计实现**——单个 `<textarea id="data">` 输入一串数字，输出「数据量/总和/平均值/中位数/最小值/最大值/极差/方差/标准差」九项。既没有竞品份额/定位矩阵，也没有投放/创意维度分析；用户按标题来用，得到的是与营销分析无关的通用统计。
  - **判定依据**：两页 `calc()` 逐字相同（仅 `<title>`/`<h1>` 不同），与 §10.6 缺陷 F 的 `clinical-lab/analysis-8`/`analysis-9`/`analysis-density-2` 为**同一份模板**；原 verify 用例的 expect 是 textarea 回显 `80_X`，即从未验证过任何计算结果。
  - **状态**：未改页面（本批为脚本批）。修法同缺陷 F：按标题重做内容，或下架并清理 `json/tools.json` + i18n 双键。**建议把「通用统计模板页」做一次全站普查**（判据：整页只有 `<textarea id="data">` 一个输入 + `calc()` 输出九项描述统计），一次性列出所有名不符实的占位页，避免每批都零散发现。
...
**第十七批成果（2026-09-18）**：pulmonology **10 例** all_default 弱用例清零 + **11 例**「expect 为注入值回显 / JS 模板残骸 / 错位」的无效用例改进 + **1 例** `no_inputs` 转真输入 + **3 例**结构性 `no_inputs` 保留（`verify_pulmonology_calc.js` 全量重写 **25/25** 通过；基线 all_default 276 → **266**、no_inputs 213 → **212**，与 selfcheck 实测一致）；判别力单文件 `discriminate_check.js verify_pulmonology_calc.js` **已检 18 / 正确变红 18 / 仍 PASS 0 / 跳过 7**（3 例 no_inputs 不适用 + 3 例动态 id select `q0..qN` + 1 例 textarea，属校验器盲区，已由自建同口径探针补验 **回退默认 FAIL 4/4**）。**10 例 all_default**：`anti-tb-dosing`（wt40/age30 → `风险因素：无`，默认 55/45 为「年龄≥35」中危）、`calc-48`（90/21 → P/F 429 `氧合功能正常`，默认 80/40 为重度 ARDS）、`feigongneng-fev1-fvc-fenji`（3.0/3.5 → `FEV1/FVC = 0.857 ≥ 0.70，无阻塞性通气障碍。`）、`light-criteria`（20/65/100/200 → `结论：漏出液`）、`niv-settings`（cardiac/10/70 → `目标潮气量 = 10 mL/kg × 70 kg = 700 mL`）、`oxygenation-index`（90/30 → `P/F = 90 / 0.30 = 300`）、`pneumothorax`（1/1/1 → `压缩＜20%且无症状，可观察吸氧`）、`pulmonary-function`（3.0/3.5/95/95 → `通气功能正常，未见阻塞性或限制性通气障碍。`）、`pulmonary-rehab`（mip30 → `吸气肌无力`，默认 45 高于下限 43 判正常）、`respiratory-failure`（80/40 → `无低氧性呼衰`）。**11 例回显/残骸/错位改进**：`lung-cancer-tnm`（`"T1b"` 为 select 回显 → T2a 断言 `分期 = ⅠB期`，须避开默认分支文案里的「ⅠB伴高危因素」）、`lung-rads`（`"14.0"` 回显 → size5 断言 `Lung-RADS 2 年度低剂量CT复查。`）、`tb-resistance`（`"1+"` 回显 → 3+/pos/rif_s 断言 `有传染性，需呼吸道隔离`）、`analysis-14`（`"80_X"` 为 textarea 回显 → 数据 `5,9,13,21,14` 断言均值 `12.40`）、`gina-asthma`（`"6/25"` 与 ACT 求和逻辑错位 → 五项全 4 断言 `20/25 ACT总分 良好控制`）、`copd-cat` 与 `self-assess-3`（inputs 键是 JS 模板残骸 `q'+i+'`、expect 是幻影元素 value 回显 → 改真键 `q0..q7`/`q0..q5` 注入 → CAT 40 分 `影响极重，多学科综合管理`、FTND 10 分 `尼古丁依赖程度极高…`）、`bronchoscopy-grading`（`tumorType=1` 实为 harness 首选项 `0` → 注入 3 → `提示浸润性癌，评估分期，可能需放化疗/手术`）、`smoking-cessation`（q1..q6 全 0 → `依赖程度轻。以行为干预为主(识别触发情境、延迟技巧)`）、`sputum-analysis`（`char=bloody` → `血性痰需排查肺结核、肺癌、支扩`）、`vibration-percussion`（`lobe=rml` → `叩击右侧乳头下方区域`）。**1 例 no_inputs 转真输入**：`rater-13`（Wells 7 项动态 id `q0..q6` 全注入「是」→ 12.5 分 → `PE概率约 49.9%`）。**3 例结构性 no_inputs 保留**：`curb65` / `stop-bang` / `wells-pe`（评分项全为 `<input type=checkbox>`，见 §10.6 缺陷 G）。**新踩坑（防复发）**：① **纯 checkbox 评分页在 harness 下不可注入，但不必一律判死**：`makeEl` 桩的 `checked` **恒为 false** 且注入只写 `.value`，`getElementById(x).checked` 永远拿不到 true（curb65/stop-bang/wells-pe 三页因此结构性 no_inputs）；但 **`c.checks` 只作用于 `querySelector(':checked')` / `querySelectorAll('…checked')`** —— 只有用选择器读取选中态的页面才可注入。② **被 checkbox 门控的页面可改判「门控前的派生量」**：`oxygenation-index` 的 `peep` 复选框门控 ARDS 分级，注入任何值都输出常量「未达评估条件」；但该页同时输出 P/F 明细，改断言 `P/F = 90 / 0.30 = 300` 即恢复判别力。③ **等级词 expect 须查「静态参考表是否恒含该串」**：`rater-13` 输出的三分类表恒含「低风险/中风险/高风险」三行，用等级词必为逃生项；只能断言随总分变化的概率串（`PE概率约 49.9%`）或总分。

**第十八批成果（2026-09-18）**：advertising **12 例** all_default 弱用例清零 + **4 例**「textarea 回显 / 依赖兜底函数」的无效用例一并重写（`verify_advertising_calc.js` 全量重写 **16/16** 通过；基线 all_default 266 → **254**，no_inputs 212 不变，与 selfcheck 实测一致）；判别力单文件 `discriminate_check.js verify_advertising_calc.js` **已检 14 / 正确变红 14 / 仍 PASS 0 / 跳过 2**（2 例 textarea 属校验器盲区，已由自建同口径探针补验 **回退默认 FAIL 2/2**）；全站 `discriminate_check.js` 已检 2362 / 变红 2362 / 逃生 **0** / 跳过 677。**12 例 all_default**：`ad-size`（600×1600 mm/96dpi → `1709mm / 6458px`，默认 1920×1080 px/300dpi 出 187mm / 2203px）、`assessor-52`（严重亏损组 → `-71.2% 活动 ROI`，默认 +248.8%）、`assessor-53`（→ `¥23,500 总触点成本` + `155% 整体 ROI`；**注意不能选「¥60,000 总转化价值」** —— 默认 120×500 也是 60000，会撞串）、`assessor-54`（→ `CPM 千次曝光成本 ¥59.52`）、`assessor-55`（→ `预估到达率 48.00%` + `有效到达率（≥4次） 15.00%`）、`color-convert`（0,114,187 → `互补色： #FF8D44` + `C:100 M:39 Y:0 K:27`）、`convert-26`（→ `264600000.000000`）、`convert-27`（→ `101640000.000000`）、`copy-duration`（14 字 + 自定义 300 字/分 → `300字/分钟 （自定义）` + `纯朗读： 2.8秒`）、`estimate-cycle`（→ `¥1,800.00 LTV` + `6.00:1 LTV:CAC 比率`）、`reach-frequency`（→ `62.3% 累计到达率` + `1,245,701 累计触达人数`）、`tester-15`（每组 4000 曝光 → `样本量偏少，建议每组至少 5000 次曝光…` + `Z 统计量 7.3003`）。**4 例其它类型**：`analysis-27` / `analysis-55`（textarea 回显 `80_X` → 数据 `5,9,13,21,14` 断言 `平均值： 12.40` + `方差： 28.64`）、`generator-time`（`cnt=12` → `12 00:`）、`storyboard-timeline`（`targetDuration=45` → `还剩45秒空间`，原断言 `还剩15秒空间` 只在兜底加载模板后才成立）。**新踩坑（防复发）**：① **等级/分类词会被兜底函数的随机态命中**：`color-convert` 用等级词「冷色」判读时，兜底 `randomColor()` 在 harness 确定性 PRNG 下恒得 `rgb(91,27,172)`（h=266°，同为冷色）→ 回退默认仍 PASS 成逃生项；改用**只由注入 RGB 派生的量**（互补色 hex / CMYK 串 / 色相角度）才安全。② **「不同输入算出同一个数」会造成撞串**：`assessor-53` 默认与改造后总转化价值都是 ¥60,000、渠道 ROI 都是 2900%（因为默认 120×500 与 300×200 恰好同积），必须换锚触点成本/整体 ROI。③ **随机生成器页别赌 PRNG**：`generator-time` 只用「第 12 行存在且时间码 <60s」这一与随机序列无关的性质（`12 00:`），不要断言具体分镜描述或时间码数字。④ **P 值类输出会被标签剥离吞掉**：`tester-15` 的 `< 0.001` 被 `collectStrings` 当标签吃掉，输出里只剩「P 值（双侧） 越小越显著」，不可作 expect。

**第十九批成果（2026-09-18）**：dyeing **13 例全量重写**（`verify_dyeing_calc.js` **13/13** 通过；基线 all_default 254 → **242**，no_inputs 212 不变，与 selfcheck 实测一致）；判别力单文件 `discriminate_check.js verify_dyeing_calc.js` **已检 13 / 正确变红 13 / 逃生 0 / 跳过 0**；全站 `discriminate_check.js` 已检 2374 / 变红 2374 / 逃生 **0** / 跳过 665。
- **本批最重大的发现 —— 一整类「常量型」逃生项**：11 例的 expect 都是 `暂无计算记录`。该串来自页面 `historyBox`，而 `saveHistory()` 写的是 **localStorage**，harness 无 localStorage 实现 → `historyBox` 恒显示占位文案，**与输入完全无关**；即注入成功与否都 PASS，13 例里 11 例（85%）中招。这是历批中命中率最高的一类。**防复发铁律**：凡页面含 `saveHistory/renderHistory/historyBox`（或任何「暂无…记录」类占位文案），`expect` 一律不得用该占位串；可先 `grep -c "暂无" tools/<slug>.html` 自查。
- **12 例 all_default 清零**（非默认输入 + Python 独立复算）：`assessor-color-diff`(CMC l:c=1:1，30/10/5→36/14/8 → `8.290` 超差-明显)、`bumianphtiaojie`(5.5→8.5/300kg → `2700.0 g` / `9.00 g/kg` / 偏离)、`calc-65`(4.00/0.40/0.20 → 上染率 `90.00%` / 固色率 `85.00%`)、`calc-dosage-2`(350/4.0/15 → `14000.00 g` / `5250.0 L` / `2.6667 g/L`)、`color-diff`(88/3.5/涤纶 → 异常 + 需调整)、`colorfast-1`(2/3/2.5 → `2.50` 级 / 最低项耐洗 / 不合格)、`ratio-21`(8/25/3/800 → 涂料 `64.0 g` / 水 `496.0 g`)、`ratio-9`(8/250/2/40 → `2000.0 L` / 染料 `5000.0 g` / 元明粉 `80000 g`)、`shuixixiaolvjisuan`(150℃/5 次/400L/10kg → `99.70%` 优秀)、`shumayinhuacanshu`(1500dpi/25m²/40%/颜料墨水 → `0.0169 mm` / `348751` / 高精)、`temp-time-humidity-1`(92℃/88%/14min/还原染料 → 异常 + 不足 + 偏长 + 需调整)、`tester-9`(A=500000/B=150 → `99.97%` / R=`3.52` / 抗菌强效)。
- **1 例回显型**：`time-35`（原 expect「viscose」是 select 值回显 → 改涤纶+分散+浅色 → `18～28` min / 中值 `23`）。
- **新踩坑（防复发）**：① **长数字的后缀会吞掉短 expect**：`ratio-9` 写 `5000.0 g 染料用量` 被默认态的 `15000.0 g 染料用量` 包含（15000.0 的后 6 位正是 5000.0）→ 回退默认仍 PASS。修法：把相邻两格合并为一条**跨格连续串**（`2000.0 L 染液总量 5000.0 g 染料用量`）。此前只记录了「避免超短串」，本例说明**光加长不够，还要防「默认态存在以它为后缀的更长数字」**。② **等级词须确认默认态不同档**：`shuixi` 原选 70℃ 得 86.77%「不足」，与默认 78.92% **同为不足**；`shuma` 原选 1200dpi「标准」，与默认 720dpi **同为标准档**（720≤dpi<1440）—— 都必须让评级**跨档**才可断言等级词（前者拉到 99.70% 优秀、后者跨到 1500dpi 高精）。`bumianphtiaojie` 同理：target 取 7.5 与默认 7.0 **同落「理想」区间**。③ **select 类输入在两道校验里口径不同**：`selfcheck._pageDefaults` 读全文匹配（取页面 JS 设定的真实默认 `auto`），`discriminate_check.pageDefaults` 取**首个 option**（占位项 `value=""`）。`bumianphtiaojie` 的 `agent` 在两种口径下分别得到 `auto` 与 `""`，但页面 `coeff`/`agentName` 用三元判断，**两者都落到「纯碱」分支** → 断言「纯碱 推荐调节剂」在校验器口径下成逃生项。修法：这类「非预期值落入同一兜底分支」的词不可作 expect，只断言由数值输入派生的量。④ **同一文件的多个 Edit 必须串行**（本批踩坑）：并行发 3 个 Edit 到同一文件出现**写回竞态** —— `ref` 注释改了但 `expect`/`inputs` 被旧快照覆盖，工具却仍报 "Successfully edited"，导致「改了却没生效」的假象，白跑两轮定位。**改完同一文件后必须 `grep` 复核关键行**。
