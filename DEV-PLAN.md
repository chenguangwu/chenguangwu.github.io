...
| # | 分类 | 工具数 | # | 分类 | 工具数 |
|---|---|---|---|---|---|
| 1 | legal | 54 | 5 | statistics | 51 |
| 2 | realestate | 54 | 6 | edu | 49 |
| 3 | meteorology | 42 | 7 | marketing | 46 |
| 4 | surveying | 44 | 8 | (按同口径递减) | — |

> 全站 209 个分类 / 4767 个工具。上表为**剩余待收口分类**（按工具数降序）；已收口的 `it`（336）、`general`（183）、`design`（109）、`finance`（104）、`science`（98）、`sports`（78）、`life`（73）、`biz`（69）、`fun`（65）、`ai`（64）、`agriculture`（63）、`hydraulic`（56）十二项已按「完成一个删一个」移除，其余分类按同口径递减。

> **收口状态（2026-09-19）**：`it` ✅ 336/336 A；`general` ✅ 183/183 A；`design` ✅ 109/109 A；`finance` ✅ 104/104 A；`science` ✅ 98/98 A；`sports` ✅ 八维审计全绿；`life`/`biz`/`fun` ✅ 八维审计全绿；`ai`/`agriculture` ✅ 八维审计全绿；`hydraulic` ✅ 八维审计全绿（cat 1 错标修 / 56/56）；`automotive` ✅ 八维审计全绿（cat 7 错标修 / 英文 i18n 全量翻译 / formula 51/51）。**下一个待收口分类 = `legal`（54 工具）。**
