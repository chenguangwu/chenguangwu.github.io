...
### ✅ media（6 工具，完整分类收口，最新一条）
- 6 工具（analysis-26 舆情情感/词频分析 / analysis-funnel 漏斗·热图数据分析 / tester-14 封面A/B / tester-18 标题A/B / tester-13 标题吸引力 / assessor-25 阅读打开率·完读评估）deep-dive 全量真实化（替换 STY3「在media业务中，先把…标准化后再执行对比」占位）。按各工具 HTML calc() 真实算法写 3 场景+1 算例+2 FAQ；算例数字均 node 实跑复核（/tmp/verify_media.js）。
- node 复核：analysis-26/analysis-funnel 实为同款描述性统计(数据量/总和/均值/中位/最小/最大/极差/方差/标准差)，情感打分[3,4,2,5,4,3,5,2,4,3]→均3.50/中位3.50/极差3/方差1.05/标准差1.02，漏斗转化[120,90,60,45,30,55,80,40,25,70]→均61.5/中位57.5/极差95/标准差27.84；assessor-25 8000/2400/1440/300/120s/800→打开30.0%/完读60.0%/互动12.5%/完成度75.0%/9分优秀；tester-14 封面10000/500 vs 10000/680→A5.00%B6.80%相对+36.0%B胜；tester-18 标题3000/120 vs 3000/210→A4.00%B7.00%相对+75.0%B胜；tester-13 微信19字/1情感词/数字3,2→6/9中等(平台阈值微信14/头条18/小红书20/抖音15)。
- analysis-26 与 analysis-funnel 标题不同但 calc() 完全相同（均为描述性统计），deep-dive 按各自业务语境（舆情打分分布 / 漏斗环节转化分布）写真实内容，未套模板；无孤儿键（6 键均有对应 HTML）。6 文件均无旧 opt-guide/opt-faq 块、无 FAQPage LD 旧套话，无需 opt 清理。
- 五道门禁全过（build+静态+死链+资产+公式回归 ALL OK）；grep 核验六型占位全 0、真实关键词命中 2–29；键数守恒 5022。脚本：scripts/apply_media.py。

## 八、当前进行中分类

> 当前无进行中分类。新分类开工时先在此登记（目录 + 工具数），收口后写入第七节并覆盖旧归档记录（§4.3 第 4 步）。

## 九、分类总清单（待办，完成一个删一个；剩 48 个目录）
