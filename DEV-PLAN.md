...
### ⏳ 进行中（Google 无查询级数据时，先用其他统计平台数据）
- [GSC-B] CTR 精修：Google 一直不收录，当前无稳定 GSC 查询级导出。先接入 Bing、百度、360 站长等平台的 query/page 统计，待 GSC 稳定后再切回。  
  - 目标：用“高展示 + 低点击率”筛选目标 URL，结合查询意图补齐 `description`，避免无差异化文案。
  - 兼容字段：`query/page/keyword`、`impressions/展示量`、`clicks/点击量`、`ctr/点击率`、`position/平均排名`
  - 已就绪：`scripts/gsc_b_candidates.py`（字段兼容、缺失告警、三类产物输出）
  - 命令：`python3 scripts/gsc_b_candidates.py --csv <查询级CSV> --min-impressions 1200 --max-ctr 0.02 --top 300`
  - 输出：
    - `gsc_b_candidates.urls.csv`
    - `gsc_b_candidates.rows.csv`
    - `gsc_b_candidates.json`
