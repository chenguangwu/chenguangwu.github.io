...
def get_tool_info(filepath):
    ...
    # Override icon/color from meta; treat 🔧 placeholder as "no override" so it
    # falls back to the category/industry icon instead of showing the wrench.
    if 'icon' in tb_meta and tb_meta['icon'] != '🔧':
        icon = tb_meta['icon']
    if 'bg' in tb_meta:
        bg = tb_meta['bg']
